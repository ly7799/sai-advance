...
ctc_greatbelt_stats_init(uint8 lchip, void* stats_global_cfg)
{
    ...
    FEATURE_SUPPORT_CHECK(lchip,CTC_FEATURE_STATS);
    LCHIP_CHECK(lchip);
    ...
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_init(lchip, (ctc_stats_global_cfg_t*)stats_global_cfg));
    }
    return CTC_E_NONE;
}
...
ctc_greatbelt_stats_deinit(uint8 lchip)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_deinit(lchip));
    }
...
ctc_greatbelt_stats_set_mac_stats_cfg(uint8 lchip, uint32 gport, ctc_mac_stats_prop_type_t mac_stats_prop_type, ctc_mac_stats_property_t prop_data)
{
    ...
    case CTC_STATS_PACKET_LENGTH_MTU1:
        CTC_ERROR_RETURN(sys_greatbelt_stats_set_mac_packet_length_mtu1(lchip, gport, prop_data.data.length));
        break;

    case CTC_STATS_PACKET_LENGTH_MTU2:
        CTC_ERROR_RETURN(sys_greatbelt_stats_set_mac_packet_length_mtu2(lchip, gport, prop_data.data.length));
        break;
...
ctc_greatbelt_stats_get_mac_stats_cfg(uint8 lchip, uint32 gport, ctc_mac_stats_prop_type_t mac_stats_prop_type, ctc_mac_stats_property_t* p_prop_data)
{
    ...
    {
    case CTC_STATS_PACKET_LENGTH_MTU1:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_mac_packet_length_mtu1(lchip, gport, &(p_prop_data->data.length)));
        break;

    case CTC_STATS_PACKET_LENGTH_MTU2:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_mac_packet_length_mtu2(lchip, gport, &(p_prop_data->data.length)));
        break;
...
ctc_greatbelt_stats_get_mac_stats(uint8 lchip, uint32 gport, ctc_mac_stats_dir_t dir, ctc_mac_stats_t* p_stats)
{
    ...
    case CTC_STATS_MAC_STATS_RX:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_mac_rx_stats(lchip, gport, p_stats));
        break;

    case CTC_STATS_MAC_STATS_TX:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_mac_tx_stats(lchip, gport, p_stats));
        break;
...
ctc_greatbelt_stats_clear_mac_stats(uint8 lchip, uint32 gport, ctc_mac_stats_dir_t dir)
{
    ...
    {
    case CTC_STATS_MAC_STATS_RX:
        CTC_ERROR_RETURN(sys_greatbelt_stats_clear_mac_rx_stats(lchip, gport));
        break;

    case CTC_STATS_MAC_STATS_TX:
        CTC_ERROR_RETURN(sys_greatbelt_stats_clear_mac_tx_stats(lchip, gport));
        break;
...
ctc_greatbelt_stats_get_cpu_mac_stats(uint8 lchip, uint32 gport, ctc_cpu_mac_stats_t* p_stats)
{
    ...
    LCHIP_CHECK(lchip);
    SYS_MAP_GPORT_TO_LCHIP(gport, lchip);
    CTC_ERROR_RETURN(sys_greatbelt_stats_get_cpu_mac_stats(lchip, gport, p_stats));
    return CTC_E_NONE;
}
...
ctc_greatbelt_stats_clear_cpu_mac_stats(uint8 lchip, uint32 gport)
{
    ...
    LCHIP_CHECK(lchip);
    SYS_MAP_GPORT_TO_LCHIP(gport, lchip);
    CTC_ERROR_RETURN(sys_greatbelt_stats_clear_cpu_mac_stats(lchip, gport));
    return CTC_E_NONE;
}
...
ctc_greatbelt_stats_set_drop_packet_stats_en(uint8 lchip, ctc_stats_discard_t bitmap, bool enable)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_set_drop_packet_stats_en(lchip, bitmap, enable));
    }
    return CTC_E_NONE;
}
...
ctc_greatbelt_stats_get_drop_packet_stats_en(uint8 lchip, ctc_stats_discard_t bitmap, bool* enable)
{
    FEATURE_SUPPORT_CHECK(lchip,CTC_FEATURE_STATS);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_greatbelt_stats_get_port_log_discard_stats_enable(lchip, bitmap, enable));
    return CTC_E_NONE;
}
...
ctc_greatbelt_stats_get_port_log_stats(uint8 lchip, uint32 gport, ctc_direction_t dir, ctc_stats_basic_t* p_stats)
{
    ...
    {
    case CTC_INGRESS:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_igs_port_log_stats(lchip, gport, p_stats));
        break;

    case CTC_EGRESS:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_egs_port_log_stats(lchip, gport, p_stats));
        break;
...
ctc_greatbelt_stats_clear_port_log_stats(uint8 lchip, uint32 gport, ctc_direction_t dir)
{
    ...
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_clear_igs_port_log_stats(lchip, gport));
    }

    if ((CTC_EGRESS == dir) || (CTC_BOTH_DIRECTION == dir))
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_clear_egs_port_log_stats(lchip, gport));
    }
...
ctc_greatbelt_stats_set_global_cfg(uint8 lchip, ctc_stats_property_param_t stats_param, ctc_stats_property_t stats_prop)
{
    ...
    case CTC_STATS_PROPERTY_SATURATE:
         CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
         {
            CTC_ERROR_RETURN(sys_greatbelt_stats_set_saturate_en(lchip, stats_param.stats_type, stats_prop.data.enable));
         }
        break;
    ...
    case CTC_STATS_PROPERTY_HOLD:
        CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
        {
            CTC_ERROR_RETURN(sys_greatbelt_stats_set_hold_en(lchip, stats_param.stats_type, stats_prop.data.enable));
        }
        break;
    ...
    case CTC_STATS_PROPERTY_PKT_CNT_THREASHOLD:
        CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
        {
            CTC_ERROR_RETURN(sys_greatbelt_stats_set_pkt_cnt_threshold(lchip, stats_prop.data.threshold_16byte));
        }
        break;
    ...
    case CTC_STATS_PROPERTY_BYTE_CNT_THREASHOLD:
        CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
        {
            CTC_ERROR_RETURN(sys_greatbelt_stats_set_byte_cnt_threshold(lchip, stats_prop.data.threshold_16byte));
        }
        break;
    ...
    case CTC_STATS_PROPERTY_FIFO_DEPTH_THREASHOLD:
        CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
        {
            CTC_ERROR_RETURN(sys_greatbelt_stats_set_fifo_depth_threshold(lchip, stats_prop.data.threshold_8byte));
        }
        break;
...
ctc_greatbelt_stats_get_global_cfg(uint8 lchip, ctc_stats_property_param_t stats_param, ctc_stats_property_t* p_stats_prop)
{
    ...
    {
    case CTC_STATS_PROPERTY_SATURATE:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_saturate_en(lchip, stats_param.stats_type, &(p_stats_prop->data.enable)));
        break;

    case CTC_STATS_PROPERTY_HOLD:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_hold_en(lchip, stats_param.stats_type, &(p_stats_prop->data.enable)));
        break;

    case CTC_STATS_PROPERTY_PKT_CNT_THREASHOLD:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_pkt_cnt_threshold(lchip, &(p_stats_prop->data.threshold_16byte)));
        break;

    case CTC_STATS_PROPERTY_BYTE_CNT_THREASHOLD:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_byte_cnt_threshold(lchip, &(p_stats_prop->data.threshold_16byte)));
        break;

    case CTC_STATS_PROPERTY_FIFO_DEPTH_THREASHOLD:
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_fifo_depth_threshold(lchip, &(p_stats_prop->data.threshold_8byte)));
        break;
...
ctc_greatbelt_stats_create_statsid(uint8 lchip, ctc_stats_statsid_t* statsid)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,
            ret,
            sys_greatbelt_stats_create_statsid(lchip, statsid));
    }

    /*rollback if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_greatbelt_stats_destroy_statsid(lchip,statsid->stats_id);
    }
...
ctc_greatbelt_stats_destroy_statsid(uint8 lchip, uint32 stats_id)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_NOT_EXIST,
            ret,
            sys_greatbelt_stats_destroy_statsid(lchip, stats_id));
    }
...
ctc_greatbelt_stats_get_stats(uint8 lchip, uint32 stats_id, ctc_stats_basic_t* p_stats)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        ...
        CTC_ERROR_RETURN(sys_greatbelt_stats_get_stats(lchip, stats_id, &stats));
...
ctc_greatbelt_stats_clear_stats(uint8 lchip, uint32 stats_id)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_clear_stats(lchip, stats_id));
    }
...
ctc_greatbelt_stats_intr_callback_func(uint8 lchip, uint8* p_gchip)
{
    ...
    CTC_ERROR_RETURN(sys_greatbelt_stats_intr_callback_func(lchip, p_gchip));
...
ctc_greatbelt_stats_register_cb(uint8 lchip, ctc_stats_sync_fn_t cb, void* userdata)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_register_cb(lchip, cb, userdata));
    }
...
ctc_greatbelt_stats_set_syncup_cb_internal(uint8 lchip, uint32 stats_interval)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_stats_set_stats_interval(lchip, stats_interval));
    }
