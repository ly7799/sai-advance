...
ctc_greatbelt_mirror_init(uint8 lchip, void* mirror_global_cfg)
{
    ...
    FEATURE_SUPPORT_CHECK(lchip,CTC_FEATURE_MIRROR);
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_mirror_init(lchip));
    }
...
ctc_greatbelt_mirror_deinit(uint8 lchip)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_mirror_deinit(lchip));
    }
...
ctc_greatbelt_mirror_set_port_en(uint8 lchip, uint32 gport, ctc_direction_t dir, bool enable, uint8 session_id)
{
    ...
    FEATURE_SUPPORT_CHECK(lchip,CTC_FEATURE_MIRROR);
    CTC_ERROR_RETURN(sys_greatbelt_mirror_set_port_en(lchip, gport, dir, enable, session_id));
...
ctc_greatbelt_mirror_get_port_info(uint8 lchip, uint32 gport, ctc_direction_t dir, bool* enable, uint8* session_id)
{
    SYS_MAP_GPORT_TO_LCHIP(gport, lchip);
    FEATURE_SUPPORT_CHECK(lchip,CTC_FEATURE_MIRROR);
    CTC_ERROR_RETURN(sys_greatbelt_mirror_get_port_info(lchip, gport, dir, enable, session_id));
...
ctc_greatbelt_mirror_set_vlan_en(uint8 lchip, uint16 vlan_id, ctc_direction_t dir, bool enable, uint8 session_id)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_mirror_set_vlan_en(lchip, vlan_id, dir, enable, session_id));
    }
...
{
    FEATURE_SUPPORT_CHECK(lchip,CTC_FEATURE_MIRROR);
    CTC_ERROR_RETURN(sys_greatbelt_mirror_get_vlan_info(lchip, vlan_id, dir, enable, session_id));
...
ctc_greatbelt_mirror_add_session(uint8 lchip, ctc_mirror_dest_t* mirror)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_NONE,ret,sys_greatbelt_mirror_set_dest(lchip, mirror));
    }

    /*rollback if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_greatbelt_mirror_unset_dest(lchip, mirror);
    }
...
ctc_greatbelt_mirror_set_escape_en(uint8 lchip, bool enable)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_mirror_rspan_escape_en(lchip, enable));
    }
    return CTC_E_NONE;
}
...
ctc_greatbelt_mirror_set_escape_mac(uint8 lchip, ctc_mirror_rspan_escape_t* escape)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_mirror_rspan_escape_mac(lchip, escape));
    }
...
ctc_greatbelt_mirror_remove_session(uint8 lchip, ctc_mirror_dest_t* mirror)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_mirror_unset_dest(lchip, mirror));
    }
...
ctc_greatbelt_mirror_set_mirror_discard(uint8 lchip, ctc_direction_t dir, uint16 discard_flag, bool enable)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_greatbelt_mirror_set_mirror_discard(lchip, dir, discard_flag, enable));
    }
...
{
    FEATURE_SUPPORT_CHECK(lchip,CTC_FEATURE_MIRROR);
    CTC_ERROR_RETURN(sys_greatbelt_mirror_get_mirror_discard(lchip, dir, discard_flag, enable));
    return CTC_E_NONE;
}
