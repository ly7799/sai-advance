...
*****************************************************************************/
STATIC int32
_sys_greatbelt_sort_key_do_alloc_offset(uint8 lchip, sys_sort_key_info_t* key_info, uint32* p_offset)
{
    ...
    CTC_ERROR_RETURN(sys_greatbelt_opf_alloc_offset(lchip, &opf, block->multiple, p_offset));
...
STATIC int32
_sys_greatbelt_sort_key_do_free_offset(uint8 lchip, sys_sort_key_info_t* key_info, uint32 offset)
{
    sys_sort_block_t* block = NULL;
    sys_greatbelt_opf_t opf;
    ...
    opf.multiple = block->multiple;
    /* 2. do it */
    CTC_ERROR_RETURN(sys_greatbelt_opf_free_offset(lchip, &opf, block->multiple, offset));
    /* adjust block info */
...
STATIC int32
_sys_greatbelt_sort_key_block_has_avil_offset(uint8 lchip, sys_sort_key_info_t* key_info, bool* p_has)
{
    sys_sort_block_t* block = NULL;
...
STATIC int32
_sys_greatbelt_sort_key_adj_block_id(uint8 lchip, sys_sort_key_info_t* key_info, sys_sort_block_dir_t dir, uint32* adj_block_id, uint8* adj_size)
{
    int32 i = 0;
    sys_sort_block_t* block = NULL;
...
STATIC int32
_sys_greatbelt_sort_key_do_block_shrink(uint8 lchip, sys_sort_key_info_t* key_info, sys_sort_block_dir_t dir)
{
    sys_sort_block_t* block = NULL;
    ...
    {
        CTC_ERROR_RETURN(sys_greatbelt_opf_alloc_offset(lchip, &opf, key_info->adj_size, &new_offset));
    ...
    if (SYS_SORT_BLOCK_DIR_UP == dir)
    {
        for (i = 0; i < adj_time; i++)
        {
            ...
            CTC_ERROR_RETURN(sys_greatbelt_opf_alloc_offset(lchip, &opf, block->multiple,  &new_offset));
    ...
    else
    {
        for (i = 0; i < adj_time; i++)
        {
            ...
            CTC_ERROR_RETURN(sys_greatbelt_opf_alloc_offset_last(lchip, &opf, block->multiple, &new_offset));
...
STATIC int32
_sys_greatbelt_sort_key_do_block_grow_with_dir(uint8 lchip, sys_sort_key_info_t* key_info, sys_sort_block_dir_t dir)
{
    sys_sort_block_t* block = NULL;
    ...
    opf.pool_type = key_info->type;
    opf.pool_index = key_info->block_id;
    CTC_ERROR_RETURN(sys_greatbelt_opf_free_offset(lchip, &opf, key_info->adj_size, desired_offset));
...
STATIC int32
_sys_greatbelt_sort_key_get_valid_block(uint8 lchip, sys_sort_key_info_t* key_info, uint16 block_id, sys_sort_block_dir_t dir, uint32* valid_block_id)
{
    int32 i = 0;
    sys_sort_block_t* block = NULL;
    block = key_info->block;
...
STATIC int32
_sys_greatbelt_sort_key_do_block_empty_grow(uint8 lchip, sys_sort_key_info_t* key_info, sys_sort_block_dir_t dir)
{
    sys_sort_block_t* block = NULL;
    ...
    ret = _sys_greatbelt_sort_key_get_valid_block(lchip, key_info, key_info->block_id, adj_dir, &valid_block_id);
    if (CTC_E_NONE != ret)
    {
        adj_dir = (SYS_SORT_BLOCK_DIR_UP == (dir) ? SYS_SORT_BLOCK_DIR_DOWN : SYS_SORT_BLOCK_DIR_UP);
        _sys_greatbelt_sort_key_get_valid_block(lchip, key_info, key_info->block_id, adj_dir, &valid_block_id);
    }
...
STATIC int32
_sys_greatbelt_sort_key_block_grow_with_dir(uint8 lchip, sys_sort_key_info_t* key_info, sys_sort_block_dir_t dir)
{
    ...
    CTC_ERROR_RETURN(_sys_greatbelt_sort_key_adj_block_id(lchip, key_info, dir, &adj_block_id, &adj_size));
    ...
    if (dir == SYS_SORT_BLOCK_DIR_DOWN)
    {
        for (i = adj_block_id; i > key_info->block_id; i--)
        {
            ...
            CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_block_shrink(lchip, &adj_key_info, adj_dir));
            adj_key_info.block_id = i - 1;
            CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_block_grow_with_dir(lchip, &adj_key_info, dir));
        }
    }
    else
    {
    ...
    if (block->all_of_num == 0)
    {
        ...
        CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_block_empty_grow(lchip, key_info, dir));
    }
...
STATIC int32
_sys_greatbelt_sort_key_block_grow(uint8 lchip, sys_sort_key_info_t* key_info)
{
    ...
    /* 2. do it */
    ret = _sys_greatbelt_sort_key_block_grow_with_dir(lchip, key_info, first_dir);
    if (CTC_E_NONE != ret)
    {
        CTC_ERROR_RETURN(_sys_greatbelt_sort_key_block_grow_with_dir(lchip, key_info, second_dir));
    }
 ...
 */
int32
sys_greatbelt_sort_key_alloc_offset(uint8 lchip, sys_sort_key_info_t* key_info, uint32* p_offset)
{
    bool has = 0;
    ...
    /* 2. do it */
    CTC_ERROR_RETURN(_sys_greatbelt_sort_key_block_has_avil_offset(lchip, key_info, &has));
    if (!has)
    {
        CTC_ERROR_RETURN(_sys_greatbelt_sort_key_block_grow(lchip, key_info));
    }
    CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_alloc_offset(lchip, key_info, p_offset));
    return CTC_E_NONE;
}
 ...
 */
int32
sys_greatbelt_sort_key_free_offset(uint8 lchip, sys_sort_key_info_t* key_info, uint32 offset)
{
    sys_sort_block_t* block = NULL;
    ...
    /* 2. do it */
    CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_free_offset(lchip, key_info, offset));
    ...
    if (offset < block->init_left_b)
    {
        ...
        CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_block_shrink(lchip, key_info, SYS_SORT_BLOCK_DIR_UP));
        CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_block_grow_with_dir(lchip, &grow_key_info, SYS_SORT_BLOCK_DIR_DOWN));
    }
    else if (offset > block->init_right_b)
    {
        ...
        CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_block_shrink(lchip, key_info, SYS_SORT_BLOCK_DIR_DOWN));
        CTC_ERROR_RETURN(_sys_greatbelt_sort_key_do_block_grow_with_dir(lchip, &grow_key_info, SYS_SORT_BLOCK_DIR_UP));
    }
...
int32
sys_greatbelt_sort_key_init_block(uint8 lchip, sys_sort_block_init_info_t* init_info)
{
    uint32 offset = 0;
    ...
    CTC_ERROR_RETURN(sys_greatbelt_opf_init_offset(lchip, init_info->opf, 0, init_info->max_offset_num));
    if (init_info->boundary_l > 0)
    {
        CTC_ERROR_RETURN(sys_greatbelt_opf_alloc_offset(lchip, init_info->opf, init_info->boundary_l, &offset));
    }

    if (init_info->boundary_r + 1 < init_info->max_offset_num)
    {
        CTC_ERROR_RETURN(sys_greatbelt_opf_alloc_offset_from_position(lchip, init_info->opf,
...
int32
sys_greatbelt_sort_key_init_offset_array(uint8 lchip, skinfo_2a* pp_offset_array, uint32 max_offset_num)
{
    uint32 num = 0, left = 0;
    uint32 alloc_num = 0;
    int32 i = 0;
