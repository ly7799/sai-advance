...
*/
#ifndef _SYS_GREATBELT_LPM_H
#define _SYS_GREATBELT_LPM_H
#ifdef __cplusplus
extern "C" {
#endif
/***************************************************************
...
#define LPM_RLT_INIT_POINTER(ipuc_ptr, ptr)                  \
    {                                                           \
        ...
        ptr->old_pointer1.idx_mask = 0xFF;                      \
                                                            \
        ptr->old_pointer2.offset = INVALID_POINTER_OFFSET;      \
        ptr->old_pointer2.stage_sram = 0xFF;                    \
        ptr->old_pointer2.idx_mask = 0xFF;                      \
                                                            \
        if (CTC_IP_VER_6 == SYS_IPUC_VER(ipuc_ptr))              \
        {                                                       \
            ptr->old_pointer3.offset = INVALID_POINTER_OFFSET;  \
            ptr->old_pointer3.stage_sram = 0xFF;                \
            ptr->old_pointer3.idx_mask = 0xFF;                  \
        }                                                       \
    }
    ...
    {                                                                           \
        ...
        {                                                                       \
            ...
            {                                                                   \
                ...
                ptr->pointer = INVALID_POINTER_OFFSET;                          \
                ptr->pointer17_16 = 0x3;                                        \
                ptr->nhp_or_cnt = 0;                                            \
            }                                                                   \
            ...
            {                                                                   \
                ...
                ptr->nhp_or_cnt = INVALID_POINTER_OFFSET;                       \
                ptr->pointer = INVALID_POINTER_OFFSET;                          \
            }                                                                   \
            else                                                                \
            {                                                                   \
...
#define TABEL_GET_STAGE(I, ipuc_ptr, ptr)                                               \
    {                                                                                   \
        uint32 IP;                                                                      \
        uint8 idx;                                                                      \
        uint8 SHIFT = 3;   /* ipv6 use 3 */                                             \
        uint8 STAGE = I;                                                                \
        uint8 masklen = (I * 8) + 32;                                                   \
        sys_l3_hash_t* p_hash;                                                          \
        ptr = NULL;                                                                     \
                                                                                        \
        if (CTC_IP_VER_4 == SYS_IPUC_VER(ipuc_ptr))                                     \
        {                                                                               \
            SHIFT = p_gb_ipuc_master[lchip]->use_hash8 ? 2 : 1;                                \
            masklen = p_gb_ipuc_master[lchip]->use_hash8 ? ((I * 8) + 8) : ((I * 8) + 16);     \
        }                                                                               \
        ...
        {                                                                               \
            _sys_greatbelt_l3_hash_get_hash_tbl(lchip, ipuc_ptr, &p_hash);                     \
            (ptr) = &(p_hash->n_table);                                                 \
        }                                                                               \
...
#define TABLE_FREE_PTR(ptr_o, idx)                                                                       \
    {                                                                                                       \
        ...
        if (IS_FREE)                                                                                         \
        {                                                                                                   \
            if (ptr_o[((idx) / (LPM_TABLE_BLOCK_NUM))])                     \
            {                                                                                           \
...
#define LPM_RLT_INIT_POINTER_STAGE(ipuc_ptr, ptr, stage)                  \
    {                                                                       \
        ...
        else if ((((ptr->old_pointer2.stage_sram >> 4) & 0xF) == stage))     \
        {                                                               \
            ptr->old_pointer2.stage_sram = 0xFF;                        \
            ptr->old_pointer2.offset = INVALID_POINTER_OFFSET;          \
            ptr->old_pointer2.stage_sram = 0xFF;                        \
        ...
        else if (CTC_IP_VER_6 == SYS_IPUC_VER(ipuc_ptr))                     \
        {                                                                   \
            if ((((ptr->old_pointer3.stage_sram >> 4) & 0xF) == stage))      \
            {                                                               \
                ptr->old_pointer3.stage_sram = 0xFF;                        \
                ptr->old_pointer3.offset = INVALID_POINTER_OFFSET;          \
                ptr->old_pointer3.stage_sram = 0xFF;                        \
...
#define LPM_RLT_GET_OFFSET(ipuc_ptr, ptr, stage)                      \
    (                                                                   \
        ...
        (ptr->old_pointer1.offset) :                                 \
        (((ptr->old_pointer2.stage_sram >> 4) & 0xF) == stage) ?         \
        (ptr->old_pointer2.offset) :                             \
        (CTC_IP_VER_6 == SYS_IPUC_VER(ipuc_ptr)) ?                       \
        ((((ptr->old_pointer3.stage_sram >> 4) & 0xF) == stage) ?      \
         (ptr->old_pointer3.offset) :                              \
         INVALID_POINTER_OFFSET                                    \
        ) : INVALID_POINTER_OFFSET                                         \
    )
...
#define LPM_RLT_GET_IDX_MASK(ipuc_ptr, ptr, stage)                     \
    (                                                                   \
        ...
        (((ptr->old_pointer1.stage_sram >> 4) & 0xF) == stage) ?         \
        (0xFF & ptr->old_pointer1.idx_mask) :                       \
        (((ptr->old_pointer2.stage_sram >> 4) & 0xF) == stage) ?          \
        (0xFF & ptr->old_pointer2.idx_mask) :                       \
        (CTC_IP_VER_6 == SYS_IPUC_VER(ipuc_ptr)) ?                       \
        ((((ptr->old_pointer3.stage_sram >> 4) & 0xF) == stage) ?      \
         (0xFF & ptr->old_pointer3.idx_mask) :                    \
         0xFF                                                      \
        ) : 0xFF                                                           \
    )
...
#define LPM_RLT_GET_SRAM_IDX(ipuc_ptr, ptr, stage)                     \
    (                                                                   \
        ...
        (((ptr->old_pointer1.stage_sram >> 4) & 0xF) == stage) ?         \
        (0x3 & ptr->old_pointer1.stage_sram) :                      \
        (((ptr->old_pointer2.stage_sram >> 4) & 0xF) == stage) ?          \
        (0x3 & ptr->old_pointer2.stage_sram) :                       \
        (CTC_IP_VER_6 == SYS_IPUC_VER(ipuc_ptr)) ?                       \
        ((((ptr->old_pointer3.stage_sram >> 4) & 0xF) == stage) ?      \
         (0x3 & ptr->old_pointer3.stage_sram) :                   \
         0x3                                                       \
        ) : 0x3                                                            \
    )
...
#define LPM_RLT_SET_OFFSET(ipuc_ptr, ptr, stage, val)                          \
    {                                                                           \
        ...
        else if ((((ptr->old_pointer2.stage_sram >> 4) & 0xF) == 0xF) ||         \
                 (((ptr->old_pointer2.stage_sram >> 4) & 0xF) == stage))              \
        {                                                                       \
            ptr->old_pointer2.stage_sram = stage << 4 |                          \
                (ptr->old_pointer2.stage_sram & 0xF);                           \
            ptr->old_pointer2.offset = val;                                     \
        }                                                                       \
        else if (CTC_IP_VER_6 == SYS_IPUC_VER(ipuc_ptr))                         \
        {                                                                       \
            if ((((ptr->old_pointer3.stage_sram >> 4) & 0xF) == 0xF) ||          \
                (((ptr->old_pointer3.stage_sram >> 4) & 0xF) == stage))          \
            {                                                                   \
                ptr->old_pointer3.stage_sram = stage << 4 |                      \
                    (ptr->old_pointer3.stage_sram & 0xF);                       \
                ptr->old_pointer3.offset = val;                                 \
            }                                                                   \
        }                                                                       \
    }
    ...
    {                                                                           \
        ...
        {                                                                       \
            ptr->old_pointer1.stage_sram = (stage << 4) | val;                   \
                 ...
                 (((ptr->old_pointer2.stage_sram >> 4) & 0xF) == stage))              \
        {                                                                       \
            ptr->old_pointer2.stage_sram = (stage << 4) | val;                   \
        }                                                                       \
        else if (CTC_IP_VER_6 == SYS_IPUC_VER(ipuc_ptr))                         \
        {                                                                       \
            if ((((ptr->old_pointer3.stage_sram >> 4) & 0xF) == 0xF) ||           \
                (((ptr->old_pointer3.stage_sram >> 4) & 0xF) == stage))          \
            {                                                                   \
                ptr->old_pointer3.stage_sram = (stage << 4) | val;               \
            }                                                                   \
        }                                                                       \
    }
...
struct sys_lpm_item_s
{
    ...
    uint16 nhp_or_cnt;      /*use to store IPv4 nexthop type hash offset or IPv6 pointer counter*/
    uint16 pointer;         /*use to store IPv4 pointer type hash offset or IPv6 pointer value 0-15 bit*/

    uint8 pointer17_16;     /*use to store IPv6 pointer value 16-17 bit*/
    uint8 rsv0[3];
};
...
struct sys_lpm_item_v4_s
{
    ...
    uint16 nhp_or_cnt;      /*use to store IPv4 nexthop type hash offset*/
    uint16 pointer;         /*use to store IPv4 pointer type hash offset*/
};
...
struct sys_lpm_tbl_s
{
    ...
    uint16 offset;      /*used to store lpm key offset, base addr of the tree */
...
struct sys_lpm_tbl_end_s
{
    ...
    uint16 offset;       /*used to store lpm key offset*/
    uint8 sram_index;    /*used to store lpm sram index*/
    uint8 index;
...
{
    uint16 offset;      /* lookupkey offset */
...
struct sys_lpm_result_s
{
    ...
    sys_lpm_pointer_info_t old_pointer2;    /* store old lookupkey info ipv4 only used 3 block*/

    uint16 hash32mid_offset;                /* store hash32 mid offset only used by ipv6*/
    uint16 hash32low_offset;                /* store hash32 low offset only used by ipv6*/
    sys_lpm_pointer_info_t old_pointer3;    /* store old lookupkey info ipv6 used 4 block*/
};
...
extern int32
_sys_greatblet_lpm_offset_alloc(uint8 lchip, uint8 sram_index, uint32 entry_num, uint32* p_offset);
