/**
 ...
 \b
 GreatBelt implements quick switching in forwarding data path while the APS protocol needs to run in a separate
 CPU if necessary. Once a failure is detected on the data path by OAM or other mechanisms,
 the CPU will be notified and it will trigger GreatBelt to perform the quick switching.
 GreatBelt implements a protocol/topology independent APS mechanism to perform quick protection switching.
 ...
 GreatBelt APS can support up to 3 level protections switching, with each level works independently.
 This is mainly used for MPLS-TP APS.
 \b
 GreatBelt support hardware based failover aps, user can use the function by set aps_failover_en when create aps
 group. Once hardware detects a port-down event, it will found whether the port is used as aps current working port,
 if this is true, the chip will change the aps protection bit itself, So very fast hardware failover times
 (on the order of less than 10 ¦Ìs) as compared to CPU/software failover(on the order of X seconds). The aps can used
 by layer 2 bridge or  mpls or ipuc, so this is Very small number of packets dropped in comparison. Notice that
 this function cannot support the aps group whose working or protection port is used as next level aps group.
 \d   For APS Bridge, the table DsApsBridge is used to make the switching.
 \d   For APS selector, the table DsApsSelect is used to make the switching.

*/

#ifndef _CTC_GREATBELT_APS_H
#define _CTC_GREATBELT_APS_H
#ifdef __cplusplus
extern "C" {
#endif
...
/**
 ...
 @param[in] aps_global_cfg

 @remark Initialize the GreatBelt aps subsystem, including global variable, soft table, asic table, etc.
...
/**
 ...
 @remark Set APS bridge group by APS group id and configure information.
                ...
                User can only update working_gport/protection_gport/w_l3if_id/p_l3if_id/next_aps/next_w_aps_en/next_p_aps_en.
