/**
 ...
 The Greatblet supports following stacking features:
\p
...
\d  Smart multicast: supports duplicating multicast packet by each node not only the ingress device to save
    ...
    ctc_stacking_set_property(ctc_stacking_property_t* p_prop) stop the multicast pakcet from assign chip.
...
\d  Flexible packet editing: supports both ingress editing and egress editing modeit mode.
...
#ifndef _CTC_GREATBELT_STK_H
#define _CTC_GREATBELT_STK_H
#ifdef __cplusplus
extern "C" {
#endif
...
/**
 @brief This function is to create a stacking uplink port trunk group

 @param[in] lchip    local chip id

 @param[in] p_trunk     stacking uplink trunk group property

 @remark  Create a stacking uplink trunk with trunk property, this function only create trunk group, will not add port.
...
/**
 @brief This function is to destroy a stacking uplink port trunk group

 @param[in] lchip    local chip id

 @param[in] p_trunk     stacking uplink trunk group property

 @remark    Destroy a stacking uplink trunk group with trunk_id
...
/**
 @brief This function is to add stacking uplink port to trunk group

 @param[in] lchip    local chip id

 @param[in] p_trunk     stacking uplink trunk group property

 @remark    Add stacking uplink port to trunk group
...
/**
 @brief This function is to remove stacking uplink port from trunk group

 @param[in] lchip    local chip id

 @param[in] p_trunk     stacking uplink trunk group property

 @remark    Remove stacking uplink port from trunk group
            gport represent stacking uplink port
...
/**
 ...
 @remark    Replace stacking trunk group's member ports with the given trunk id and gports.
...
/**
 ...
 @remark    Get trunk member ports

 @return CTC_E_XXX
*/
extern int32
ctc_greatbelt_stacking_get_member_ports(uint8 lchip_id, uint8 trunk_id, uint32* p_gports, uint8* cnt);


/**
 @brief This function is to add remote device chipid to trunk group

 @param[in] lchip    local chip id

 @param[in] p_trunk     stacking uplink trunk group property

 @remark    Add stacking remote device chipid to trunk group which used for unicast uplink select
...
/**
 @brief This function is to remove remote device chipid from trunk group

 @param[in] lchip    local chip id

 @param[in] p_trunk     stacking uplink trunk group property

 @remark    Remove stacking remote device chipid from trunk group which used for unicast uplink select
            remote_gchip represent remote device chipid
...
/**
 ...
 @remark    Get stacking trunk id from remote device chipid which used for unicast uplink select
            remote_gchip represent remote device chipid
...
/**
 @brief This function is to set property of stacking

 @param[in] lchip    local chip id

 @param[in] p_prop   point to stacking property

 @remark    set property of stacking
...
/**
 @brief This function is to get property of stacking

 @param[in] lchip    local chip id

 @param[in] p_prop   point to stacking property

 @remark    get property of stacking
            CTC_STK_PROP_REMOTE_CHIP_END_BITMAP:
                 notify from the remote chip mcast packet will be end
            CTC_STK_PROP_BREAK_EN:
                 notify stacking exist break point
...
/**
 ...
 @remark  the keeplive group id is the global mcast group id, so the user must maintain the group id
...
/**
 @brief This function is to add member to cpu-to-cpu keeplive group

 @param[in] lchip    local chip id

 @param[in] p_keeplive   point to keeplive member

 @remark  cpu-to-cpu keeplive packet can go through from the assing trunk which the member
...
/**
 @brief This function is to remove member to cpu-to-cpu keeplive group
...
/**
 ...
 @remark  Get keeplive group members.
...
/**
 ...
 @remark  set trunk mcast group  profile which use to replicate packet to remote device
...
/**
 ...
 @remark  get trunk mcast group  profile which use to replicate packet to remote device
...
/**
 ...
 @remark  User can de-initialize the stacking configuration
