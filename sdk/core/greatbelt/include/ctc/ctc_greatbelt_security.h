/**
 ...
 \p
 Enable or disable mac limit on a port or a vlan or system. Based on port, the function will be enabled on port by API
 ...
 source address, ip source address, vlan id and their any combination with souce port equal to the entry configured by API
 ctc_ip_source_guard_add_entry(). If compared correctly, the packet will be forwarding nomally. Otherwise, the packet
 will be reported as a security violation event to discard. Enable or disable the function on port by API ctc_port_set_scl_key_type()
 with key type relative.
 \p
 ...
 limit on edge point of network. Greatbelt support port and vlan base storm control and total is 63 controls entry which port and vlan shares
 Strom control can limit rate by bps and bps and its grain is 1kbps or pbs. Before use api set storm control limit rate, it's recommend to set global
 config by ctc_storm_ctl_set_global_cfg().
 \p
 ...
 If they are in the same isolated group, the souce port and destination port cannot communicate with each other.
 Further more, isolation type of packet can be including: unknown ucast, unknown mcast. You can set type in struct ctc_port_restriction_t.
 ...
 \p
 There are two ways to support PVlan in GreatBelt.
 \p
 \d    The first way is introduced in GreatBelt, which is easy to configure and will not consume extra resource. But the limitation is that it can't support trunk port.
       You must configure port type that can be promiscuous, isolated, community and none(nomal port) by API ctc_port_set_restriction(). Then, if the port is
        community port, the isolated group id must be set as isolated_id by API ctc_port_set_restriction().
 \d    The other way is the same with Bay and Humber, which can support trunk port but with more complicated configuration, including that it requires asymmetric vlan,
       egress vlan filter, etc. The second way is using egress vlan filtering to support PVLAN in GreatBelt. But isolated port only support that incoming
       packet is untag packet.
 \p
 Blocking based on port is a security feature. By default, the switch floods all packets including unknown unicast, unknown multicast, known multicast and broadcast. If
 unknown unicast, unknown multicast, known multicast or broadcast traffic is forwarded to a protected port, there could be security issues. To
 prevent unknown unicast, unknown multicast, known multicast or broadcast traffic from being forwarded from one port to another, you can use
...
#ifndef _CTC_GREATBELT_SECURITY_H
#define _CTC_GREATBELT_SECURITY_H
#ifdef __cplusplus
extern "C" {
#endif
...
*/
extern int32
ctc_greatbelt_security_init(uint8 lchip, void* security_global_cfg);
...
/**
 ...
 @remark  User can de-initialize the security configuration

 @return CTC_E_XXX
*/
extern int32
ctc_greatbelt_security_deinit(uint8 lchip);
...
/**
 ...
 @remark
       ...
       and not learning this mac address again when the souce port is mismatch with FDB .
...
*/
extern int32
ctc_greatbelt_mac_security_set_port_security(uint8 lchip, uint32 gport, bool enable);
...
*/
extern int32
ctc_greatbelt_mac_security_get_port_security(uint8 lchip, uint32 gport, bool* p_enable);
...
*/
extern int32
ctc_greatbelt_mac_security_set_port_mac_limit(uint8 lchip, uint32 gport, ctc_maclimit_action_t action);
/**
...
*/
extern int32
ctc_greatbelt_mac_security_get_port_mac_limit(uint8 lchip, uint32 gport, ctc_maclimit_action_t* action);
...
*/
extern int32
ctc_greatbelt_mac_security_set_vlan_mac_limit(uint8 lchip, uint16 vlan_id, ctc_maclimit_action_t action);
...
*/
extern int32
ctc_greatbelt_mac_security_get_vlan_mac_limit(uint8 lchip, uint16 vlan_id, ctc_maclimit_action_t* action);
...
*/
extern int32
ctc_greatbelt_mac_security_set_system_mac_limit(uint8 lchip, ctc_maclimit_action_t action);
/**
...
*/
extern int32
ctc_greatbelt_mac_security_get_system_mac_limit(uint8 lchip, ctc_maclimit_action_t* action);
...
*/
extern int32
ctc_greatbelt_mac_security_set_learn_limit(uint8 lchip, ctc_security_learn_limit_t* p_learn_limit);
...
*/
extern int32
ctc_greatbelt_mac_security_get_learn_limit(uint8 lchip, ctc_security_learn_limit_t* p_learn_limit);
...
/**
 ...
 @remark
     ...
     6 kind of ip source guard check mode is support in GB. The mode can be configured by parameter ip_src_guard_type.
     \p
     ...
     \p
     And you must enable ip source guard function on port by API ctc_port_set_scl_key_type(), according to ip source guard check mode.
     You can see enum ctc_port_scl_key_type_t note(CTC_SCL_KEY_TYPE_IPSG_PORT_MAC and CTC_SCL_KEY_TYPE_IPSG_PORT_IP).

 @return CTC_E_XXX

*/
extern int32
ctc_greatbelt_ip_source_guard_add_entry(uint8 lchip, ctc_security_ip_source_guard_elem_t* p_elem);
...
/**
 ...
 @remark
     ...
     CTC_SECURITY_BINDING_TYPE_MAC_VLAN:           mac_sa and vid and gport must be configured.
     \p
     If you want to disable ip source guard function on the gport, it must be set disable by API ctc_port_set_scl_key_type().
     You can see enum ctc_port_scl_key_type_t note(CTC_SCL_KEY_TYPE_DISABLE).

 @return CTC_E_XXX

*/
extern int32
ctc_greatbelt_ip_source_guard_remove_entry(uint8 lchip, ctc_security_ip_source_guard_elem_t* p_elem);
...
/**
 ...
 @remark
  ...
  can cotrol packets such as:
  \p
  CTC_SECURITY_STORM_CTL_BCAST        : broadcast storm control.
  \p
  CTC_SECURITY_STORM_CTL_KNOWN_MCAST_ : known multicast storm control.
  \p
  CTC_SECURITY_STORM_CTL_UNKNOWN_MCAST: unknown multicast storm control.
  \p
  CTC_SECURITY_STORM_CTL_ALL_MCAST_   : known and unknown multicast storm control.
  \p
  CTC_SECURITY_STORM_CTL_KNOWN_UCAST_ : known unicast storm control.
  \p
...
*/
extern int32
ctc_greatbelt_storm_ctl_set_cfg(uint8 lchip, ctc_security_stmctl_cfg_t* stmctl_cfg);
...
*/
extern int32
ctc_greatbelt_storm_ctl_get_cfg(uint8 lchip, ctc_security_stmctl_cfg_t* stmctl_cfg);
...
*/
extern int32
ctc_greatbelt_storm_ctl_set_global_cfg(uint8 lchip, ctc_security_stmctl_glb_cfg_t* p_glb_cfg);
...
*/
extern int32
ctc_greatbelt_storm_ctl_get_global_cfg(uint8 lchip, ctc_security_stmctl_glb_cfg_t* p_glb_cfg);
...
*/
extern int32
ctc_greatbelt_port_isolation_set_route_obey_isolated_en(uint8 lchip, bool enable);
...
*/
extern int32
ctc_greatbelt_port_isolation_get_route_obey_isolated_en(uint8 lchip, bool* p_enable);
