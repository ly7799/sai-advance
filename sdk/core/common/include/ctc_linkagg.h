...
enum ctc_linkagg_property_e
{
    ...
    CTC_LINKAGG_PROP_LMPF_LB_HASH_OFFSET,    /**< [TM] linkagg local member first load balance hash offset */
    CTC_LINKAGG_PROP_LMPF_CANCEL             /**< [D2.TM]if enable, cancel local member priority first group ,and use the common group*/
};
typedef enum ctc_linkagg_property_e ctc_linkagg_property_t;


/**
 @brief linkagg port array information
*/
struct ctc_linkagg_port_s
{
    uint32 gport;             /**< [D2.TM] gport id */
    uint8  lmpf_en;           /**< [D2.TM] if set , it will join in local member first group*/
};
typedef struct ctc_linkagg_port_s ctc_linkagg_port_t;
...
enum  ctc_linkagg_psc_mpls_header_e
{
    ...
    CTC_LINKAGG_PSC_MPLS_CANCEL_LABEL = 0x00000010   /**< [GG.D2] cancel mpls label to compute linkagg hash flags*/
};
...
enum ctc_linkagg_psc_type_e
{
    ...
    CTC_LINKAGG_PSC_TYPE_L2_ONLY= 0x02000000,    /**< [TM] if set, only care about l2 info*/
};
