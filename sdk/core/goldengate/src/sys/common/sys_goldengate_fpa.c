...
#define DO_REORDER(move_num, all)    (((move_num)>32)&&((move_num) > ((all) / TRIGGER_REORDER)))
 ...
 */
STATIC int32
_fpa_goldengate_entry_move(ctc_fpa_t* fpa, ctc_fpa_entry_t* pe, int32 amount)
{
 ...
 */
STATIC int32
_fpa_goldengate_entry_shift_up(ctc_fpa_t* fpa,
                    ctc_fpa_block_t* pb,
                    int32 target_index,
                    ...
                    uint32* move_num)
{
    ...
    while ((temp < target_index))
    {
        ...
        /* Perform entry move. */
        CTC_ERROR_RETURN(_fpa_goldengate_entry_move(fpa, pb->entries[temp + step], -step));
 ...
 */
STATIC int32
_fpa_goldengate_entry_shift_down(ctc_fpa_t* fpa,
                      ctc_fpa_block_t* pb,
                      int32 target_index,
                      ...
                      uint32* move_num)
{
    ...
    temp = (next_null_index - target_index)/step;
    *move_num = (temp < 0) ? 0 : temp;
 ...
 */
STATIC bool
_fpa_goldengate_get_shift_count(int32 null, int32 target, int32 dir, int32* count)
{
    if (-1 == dir)
    {
 ...
 */
STATIC int32
_fpa_goldengate_get_shift_direction(int32 prev_null_index,
                         int32 target_index,
                         int32 next_null_index,
                         int32* dir)
{
    ...
    shift_up = _fpa_goldengate_get_shift_count(prev_null_index, target_index, -1, &shift_up_amount);

    shift_down = _fpa_goldengate_get_shift_count(next_null_index, target_index, 1, &shift_down_amount);
...
STATIC int32
_fpa_goldengate_reorder(ctc_fpa_t* fpa, ctc_fpa_block_t* pb, uint8 key_size, uint8 type, int16 start_move_num, int16 sub_count_vary)
{
    int32         idx;
    int32         revise;
    int32         t_idx;
        ...
        for (idx = 0; idx < left_num; idx++)
        {
            ...
            else
            {
                ...
                if (move_ok)
                {
                    ...
                    /* move revise to temp */
                    CTC_ERROR_GOTO(_fpa_goldengate_entry_move(fpa, pb->entries[target_a[revise].o_idx],
...
*/
STATIC int32
_fpa_goldengate_get_indexes(uint32 prio, ctc_fpa_entry_t* pe, ctc_fpa_block_t* pb, uint8 key_size,
...
STATIC int32
_fpa_goldengate_get_offset(ctc_fpa_t* fpa, ctc_fpa_entry_t* pe, ctc_fpa_block_t* pb, uint8 key_size,
                uint32 prio, int32* target, uint32* move_num_out)
{
    ...
    _fpa_goldengate_get_indexes(prio, pe, pb, key_size, &targ, &prev, &next,
    ...
    if (FPA_INVALID_INDEX == targ)
    {
        ...
        else if (SUB_BLOCK_END(step) == last_valid)    /*last valid entry in the end of block*/
        {
            ...
            CTC_ERROR_RETURN(_fpa_goldengate_entry_shift_up(fpa, pb, target_out, prev, step, &move_num));
        }
        else
        {
    ...
    else if(first_valid == targ)
    {
        if(SUB_BLOCK_START == targ)   /*in the start of the block*/
        {
            ...
            CTC_ERROR_RETURN(_fpa_goldengate_entry_shift_down(fpa, pb, target_out, next, step, &move_num));
        }
        else
        {
    ...
    {
        _fpa_goldengate_get_shift_direction(prev, targ, next, &dir);
        if (dir == 1)
        {
            target_out = targ;
            /*
...
STATIC int32
_fpa_adjust_resource(ctc_fpa_t* fpa, ctc_fpa_block_t* pb, uint8 key_size)
{
    int8   idx1 = 0;
    int8   idx2 = 0;
    uint8  step = 0;
    ...
    require_step = _fpa_get_step(key_size);
    ...
    if((up_free_count >= down_free_count) && (up_free_count >= require_step))
    {
        ...
        for(idx1 = key_size - 1; idx1 >= 0; idx1--)
        {
            ...
            if(FPA_FREE_COUNT(idx1) * step >= require_step)
            {
                ...
                _fpa_goldengate_reorder(fpa, pb, idx1, REORDE_TYPE_SCATTER, 0, -(update_entry_count / step));
                ...
                {
                    _fpa_goldengate_reorder(fpa, pb, idx2, REORDE_TYPE_DECREASE, -update_entry_count, 0);    /*move up*/
    ...
    else if(down_free_count >= require_step)
    {
        ...
        for(idx1 = key_size + 1; idx1 < CTC_FPA_KEY_SIZE_NUM; idx1++)
        {
            ...
            if(FPA_FREE_COUNT(idx1) > 0)
            {
                ...
                _fpa_goldengate_reorder(fpa, pb, idx1, REORDE_TYPE_SCATTER, update_entry_count, -(update_entry_count / step));
                pb->sub_entry_count[idx1] -= (update_entry_count / step);
                pb->sub_free_count[idx1] -= (update_entry_count / step);
                pb->start_offset[idx1] += update_entry_count;
                for(idx2 = idx1 - 1; idx2 > key_size; idx2--)
                {
                    _fpa_goldengate_reorder(fpa, pb, idx2, REORDE_TYPE_INCREASE, update_entry_count, 0);    /*move down*/
                    pb->start_offset[idx2] += update_entry_count;
                }
                pb->sub_entry_count[key_size] += (update_entry_count / require_step);
                pb->sub_free_count[key_size] += (update_entry_count / require_step);
...
fpa_goldengate_set_entry_prio(ctc_fpa_t* fpa, ctc_fpa_entry_t* pe, ctc_fpa_block_t* pb, uint8 key_size, uint32 prio)
{
    ...
    else if(0 == pb->sub_free_count[key_size])
    {
        CTC_ERROR_RETURN(_fpa_adjust_resource(fpa, pb, key_size));
        pb->decrease_trend[key_size]  = 0;
        pb->increase_trend[key_size]  = 0;
        pb->trend_active[key_size]    = 0;
    }

    CTC_ERROR_RETURN(_fpa_goldengate_get_offset(fpa, pe, pb, key_size, prio, &target, &move_num));
    temp  = target;
    temp1 = pe->offset_a;
    ...
    if ((temp - temp1) != 0)
    {
        ...
        CTC_ERROR_RETURN(_fpa_goldengate_entry_move(fpa, pe, (temp - temp1)));
    }
    ...
    {
        CTC_ERROR_RETURN(_fpa_goldengate_reorder(fpa, pb, key_size, REORDE_TYPE_SCATTER, 0, 0));
    }
...
fpa_goldengate_alloc_offset(ctc_fpa_t* fpa, ctc_fpa_block_t* pb, uint8 key_size, uint32 prio, uint32* block_index)
{
    ...
    {
        CTC_ERROR_RETURN(_fpa_goldengate_reorder(fpa, pb, key_size, REORDE_TYPE_DECREASE, 0, 0));
    }
    else if (DO_PRESS(pb->increase_trend[key_size], pb->sub_entry_count[key_size]))
    {
        CTC_ERROR_RETURN(_fpa_goldengate_reorder(fpa, pb, key_size, REORDE_TYPE_INCREASE, 0, 0));
    }

    CTC_ERROR_RETURN(_fpa_goldengate_get_offset(fpa, NULL, pb, key_size, prio, &target, &pb->move_num));
