...
int32
sys_goldengate_chip_set_active(uint8 lchip, uint32 value)
{
    ...
    if (value)
    {
        ...
        {
            CTC_BIT_SET(goldengate_chip_valid[lchip/SYS_CHIP_BITMAP_NUM], lchip%SYS_CHIP_BITMAP_NUM);
        }
    }
    else
    {
        if (CTC_IS_BIT_SET(goldengate_chip_valid[lchip/SYS_CHIP_BITMAP_NUM], lchip%SYS_CHIP_BITMAP_NUM))
        {
            CTC_BIT_UNSET(goldengate_chip_valid[lchip/SYS_CHIP_BITMAP_NUM], lchip%SYS_CHIP_BITMAP_NUM);
        }
        else
        {
