...
ctc_goldengate_qos_init(uint8 lchip, void* p_glb_parm)
{
        ...
        || !sal_memcmp(&(((ctc_qos_global_cfg_t *)p_glb_parm)->resrc_pool), p_resrc_pool_0, sizeof(ctc_qos_resrc_pool_cfg_t)))
    {
        ...
        if (NULL == p_glb_parm)
        {
            ...
            p_glb_parm_tmp->policer_num = 4096;
        ...
        p_glb_parm_tmp->resrc_pool.egs_pool_size[CTC_QOS_EGS_RESRC_DEFAULT_POOL]  = 215*1024/10;
        ...
        p_glb_parm_tmp->resrc_pool.egs_pool_size[CTC_QOS_EGS_RESRC_CONTROL_POOL]  = 2*1024;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_NONE,
                                ret,
                                sys_goldengate_qos_init(lchip, p_glb_parm));
    }
...
ctc_goldengate_qos_deinit(uint8 lchip)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_deinit(lchip));
    }
...
*/
STATIC int32
_ctc_goldengate_get_lchip_from_queue(ctc_qos_queue_id_t queue, uint8* lchip, uint8* all_lchip)
{
...
ctc_goldengate_qos_set_policer(uint8 lchip, ctc_qos_policer_t* p_policer)
{
    ...
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_policer(lchip, p_policer));
    }
...
ctc_goldengate_qos_get_policer(uint8 lchip, ctc_qos_policer_t* p_policer)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_get_policer(lchip, p_policer));
    }
...
ctc_goldengate_qos_set_global_config(uint8 lchip, ctc_qos_glb_cfg_t* p_glb_cfg)
{
    ...
    case CTC_QOS_GLB_CFG_QUEUE_DROP_MONITOR_EN:
        ...
        _ctc_goldengate_get_lchip_from_queue(queue, &lchip, &all_lchip);
        break;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_global_config(lchip, p_glb_cfg));
    }
...
ctc_goldengate_qos_get_global_config(uint8 lchip, ctc_qos_glb_cfg_t* p_glb_cfg)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_qos_get_global_config(lchip, p_glb_cfg));
...
ctc_goldengate_qos_set_domain_map(uint8 lchip, ctc_qos_domain_map_t* p_domain_map)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_domain_map(lchip, p_domain_map));
    }
...
ctc_goldengate_qos_get_domain_map(uint8 lchip, ctc_qos_domain_map_t* p_domain_map)
{
    ...
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_QOS);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_qos_get_domain_map(lchip, p_domain_map));
...
ctc_goldengate_qos_set_queue(uint8 lchip, ctc_qos_queue_cfg_t* p_que_cfg)
{
    ...
    case CTC_QOS_QUEUE_CFG_QUEUE_STATS_EN:
        _ctc_goldengate_get_lchip_from_queue(p_que_cfg->value.stats.queue, &lchip, &all_lchip);
        break;

    case CTC_QOS_QUEUE_CFG_LENGTH_ADJUST:
        _ctc_goldengate_get_lchip_from_queue(p_que_cfg->value.pkt.queue, &lchip, &all_lchip);
        break;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_queue(lchip, p_que_cfg));
    }
...
ctc_goldengate_qos_set_shape(uint8 lchip, ctc_qos_shape_t* p_shape)
{
    ...
    case CTC_QOS_SHAPE_QUEUE:
        _ctc_goldengate_get_lchip_from_queue(p_shape->shape.queue_shape.queue, &lchip, &all_lchip);
        break;

    case CTC_QOS_SHAPE_GROUP:
        _ctc_goldengate_get_lchip_from_queue(p_shape->shape.group_shape.queue, &lchip, &all_lchip);
        break;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_shape(lchip, p_shape));
    }
...
ctc_goldengate_qos_set_sched(uint8 lchip, ctc_qos_sched_t* p_sched)
{
    ...
    case CTC_QOS_SCHED_QUEUE:
        _ctc_goldengate_get_lchip_from_queue(p_sched->sched.queue_sched.queue, &lchip, &all_lchip);
        break;

    case CTC_QOS_SCHED_GROUP:
        _ctc_goldengate_get_lchip_from_queue(p_sched->sched.group_sched.queue, &lchip, &all_lchip);
        break;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_sched(lchip, p_sched));
    }
...
ctc_goldengate_qos_get_sched(uint8 lchip, ctc_qos_sched_t* p_sched)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_get_sched(lchip, p_sched));
    }
...
ctc_goldengate_qos_set_drop_scheme(uint8 lchip, ctc_qos_drop_t* p_drop)
{
    ...
    _ctc_goldengate_get_lchip_from_queue(p_drop->queue, &lchip, &all_lchip);
    lchip_start = lchip;
    lchip_end = lchip + 1;

    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_drop_scheme(lchip, p_drop));
    }
...
ctc_goldengate_qos_get_drop_scheme(uint8 lchip, ctc_qos_drop_t* p_drop)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_qos_get_drop_scheme(lchip, p_drop));
...
ctc_goldengate_qos_set_resrc(uint8 lchip, ctc_qos_resrc_t* p_resrc)
{
    ...
    case CTC_QOS_RESRC_CFG_QUEUE_DROP:
        _ctc_goldengate_get_lchip_from_queue(p_resrc->u.queue_drop[0].queue,
                                             &lchip, &all_lchip);
        break;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_set_resrc(lchip, p_resrc));
    }
...
ctc_goldengate_qos_get_resrc(uint8 lchip, ctc_qos_resrc_t* p_resrc)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_get_resrc(lchip, p_resrc));
    }
...
ctc_goldengate_qos_query_pool_stats(uint8 lchip, ctc_qos_resrc_pool_stats_t* p_stats)
{
    ...
    {
        _ctc_goldengate_get_lchip_from_queue(p_stats->queue, &lchip, &all_lchip);
    }
    else
    {
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_query_pool_stats(lchip, p_stats));
    }
...
ctc_goldengate_qos_query_queue_stats(uint8 lchip, ctc_qos_queue_stats_t* p_queue_stats)
{
    ...
    _ctc_goldengate_get_lchip_from_queue(p_queue_stats->queue, &lchip, &all_lchip);
    lchip_start = lchip;
    lchip_end = lchip + 1;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_query_queue_stats(lchip, p_queue_stats));
...
ctc_goldengate_qos_clear_queue_stats(uint8 lchip, ctc_qos_queue_stats_t* p_queue_stats)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_clear_queue_stats(lchip, p_queue_stats));
    }
...
ctc_goldengate_qos_query_policer_stats(uint8 lchip, ctc_qos_policer_stats_t* p_policer_stats)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_query_policer_stats(lchip, p_policer_stats));
...
ctc_goldengate_qos_clear_policer_stats(uint8 lchip, ctc_qos_policer_stats_t* p_policer_stats)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_qos_clear_policer_stats(lchip, p_policer_stats));
    }
