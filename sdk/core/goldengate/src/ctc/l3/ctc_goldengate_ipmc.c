...
ctc_goldengate_ipmc_init(uint8 lchip, void* ipmc_global_cfg)
{
    ...
    {
        CTC_ERROR_RETURN(sys_goldengate_ipmc_init(lchip));
    }
...
ctc_goldengate_ipmc_deinit(uint8 lchip)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_ipmc_deinit(lchip));
    }
...
ctc_goldengate_ipmc_add_group(uint8 lchip, ctc_ipmc_group_info_t* p_group)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_IPMC_GROUP_HAS_EXISTED,
            ret,
            sys_goldengate_ipmc_add_group(lchip, p_group));
    }

    /* rollback if error exist */
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_ipmc_remove_group(lchip, p_group);
    }
...
ctc_goldengate_ipmc_remove_group(uint8 lchip, ctc_ipmc_group_info_t* p_group)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_IPMC_GROUP_NOT_EXIST,
            ret,
            sys_goldengate_ipmc_remove_group(lchip, p_group));
    }
...
*/
int32
ctc_goldengate_ipmc_add_member(uint8 lchip, ctc_ipmc_group_info_t* p_group_para)
{
    uint8 lchip_start  = 0;
    uint8 lchip_end    = 0;
    int32 ret          = CTC_E_NONE;
    ...
    if(!pold_member_info_ptr || !pipmc_member || !pipmc_reroute_member || !p_group)
    {
        ret = CTC_E_NO_MEMORY;
        goto out;
    }
    ...
    sal_memcpy(p_group, p_group_para, sizeof(ctc_ipmc_group_info_t));
    ...
    {
        if(p_group->ipmc_member[index].is_nh && sys_goldengate_nh_get_nhinfo(lchip,
            p_group->ipmc_member[index].nh_id,&sys_nh_info) < 0)
        {
            reroute_member = 0;
    ...
    if(reroute_member)
    {
        ...
        CTC_FOREACH_LCHIP(lchip_start, lchip_end, 0)
        {
            CTC_FOREACH_ERROR_RETURN(CTC_E_NONE,
                ret,
                sys_goldengate_ipmc_add_member(lchip, p_group));
        }
...
ctc_goldengate_ipmc_update_rpf(uint8 lchip, ctc_ipmc_group_info_t* p_group)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_ipmc_update_rpf(lchip, p_group));
    }
...
ctc_goldengate_ipmc_add_rp_intf(uint8 lchip, ctc_ipmc_rp_t * p_rp)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_NONE,
            ret,
            sys_goldengate_ipmc_add_rp_intf(lchip, p_rp));
    }

    /* rollback if error exist */
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_ipmc_remove_rp_intf(lchip, p_rp);
    }
...
ctc_goldengate_ipmc_get_group_info(uint8 lchip, ctc_ipmc_group_info_t* p_group)
{
    ...
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_IPMC);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_ipmc_get_group_info(lchip, p_group));
...
ctc_goldengate_ipmc_add_default_entry(uint8 lchip, uint8 ip_version, ctc_ipmc_default_action_t type)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_ipmc_add_default_entry(lchip, ip_version, type));
    }
...
ctc_goldengate_ipmc_set_mcast_force_route(uint8 lchip, ctc_ipmc_force_route_t* p_data)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_ipmc_set_mcast_force_route(lchip, p_data));
    }
...
ctc_goldengate_ipmc_get_mcast_force_route(uint8 lchip, ctc_ipmc_force_route_t* p_data)
{
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_IPMC);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_ipmc_get_mcast_force_route(lchip, p_data));
