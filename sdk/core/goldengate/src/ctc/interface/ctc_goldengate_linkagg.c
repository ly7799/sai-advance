...
ctc_goldengate_linkagg_init(uint8 lchip, void* linkagg_global_cfg)
{
    ...
    {
        CTC_ERROR_RETURN(sys_goldengate_linkagg_init(lchip, linkagg_global_cfg));
    }
...
ctc_goldengate_linkagg_deinit(uint8 lchip)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_linkagg_deinit(lchip));
    }
...
ctc_goldengate_linkagg_create(uint8 lchip, ctc_linkagg_group_t* p_linkagg_grp)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_LINKAGG_HAS_EXIST,
            ret,
            sys_goldengate_linkagg_create(lchip, p_linkagg_grp));
    }

    /*rollback if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_linkagg_destroy(lchip,p_linkagg_grp->tid);
    }
...
ctc_goldengate_linkagg_destroy(uint8 lchip, uint8 tid)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_LINKAGG_NOT_EXIST,
            ret,
            sys_goldengate_linkagg_destroy(lchip, tid));
    }
...
ctc_goldengate_linkagg_add_port(uint8 lchip, uint8 tid, uint32 gport)
{
    ...
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_linkagg_remove_port(lchip, tid, gport);
    }
...
ctc_goldengate_linkagg_replace_ports(uint8 lchip, uint8 tid, uint32* gport, uint16 mem_num)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_linkagg_replace_ports(lchip, tid, gport, mem_num));
    }
...
ctc_goldengate_linkagg_get_1st_local_port(uint8 lchip_id, uint8 tid, uint32* p_gport, uint16* local_cnt)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_linkagg_get_1st_local_port(lchip_id, tid, p_gport, local_cnt));
...
ctc_goldengate_linkagg_get_member_ports(uint8 lchip, uint8 tid, uint32* p_gports, uint16* cnt)
{
    ...
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_LINKAGG);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_linkagg_show_ports(lchip, tid, p_gports, cnt));
...
*/
int32
ctc_goldengate_linkagg_set_psc(uint8 lchip, ctc_linkagg_psc_t* p_psc)
{
    uint8 lchip_start = 0;
    uint8 lchip_end = 0;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_linkagg_set_psc(lchip, p_psc));
    }
...
*/
int32
ctc_goldengate_linkagg_get_psc(uint8 lchip, ctc_linkagg_psc_t* p_psc)
{

    FEATURE_SUPPORT_CHECK(CTC_FEATURE_LINKAGG);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_linkagg_get_psc(lchip, p_psc));
...
ctc_goldengate_linkagg_get_max_mem_num(uint8 lchip, uint16* max_num)
{
    ...
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_LINKAGG);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_linkagg_get_max_mem_num(lchip, max_num));
