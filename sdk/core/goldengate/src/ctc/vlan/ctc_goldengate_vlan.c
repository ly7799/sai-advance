...
int32
ctc_goldengate_vlan_deinit(uint8 lchip)
{
    uint8 lchip_start               = 0;
    uint8 lchip_end                 = 0;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_vlan_mapping_deinit(lchip));
        CTC_ERROR_RETURN(sys_goldengate_vlan_class_deinit(lchip));
        CTC_ERROR_RETURN(sys_goldengate_vlan_deinit(lchip));
    }
...
ctc_goldengate_vlan_create_vlan(uint8 lchip, uint16 vlan_id)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_VLAN_EXIST,ret,
                            sys_goldengate_vlan_create_vlan(lchip, vlan_id));
    }

    /*roll back if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_vlan_destory_vlan(lchip,vlan_id);
    }
...
ctc_goldengate_vlan_create_uservlan(uint8 lchip, ctc_vlan_uservlan_t* user_vlan)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_VLAN_EXIST,ret,
                                 sys_goldengate_vlan_create_uservlan(lchip, user_vlan));
    }
...
ctc_goldengate_vlan_destroy_vlan(uint8 lchip, uint16 vlan_id)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_VLAN_NOT_CREATE,ret,
                                        sys_goldengate_vlan_destory_vlan(lchip, vlan_id));
    }
...
*/
int32
ctc_goldengate_vlan_add_ports(uint8 lchip, uint16 vlan_id, ctc_port_bitmap_t port_bitmap)
{
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_VLAN);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_vlan_add_ports(lchip, vlan_id, port_bitmap));
...
*/
int32
ctc_goldengate_vlan_remove_ports(uint8 lchip, uint16 vlan_id, ctc_port_bitmap_t port_bitmap)
{
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_VLAN);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_vlan_remove_ports(lchip, vlan_id, port_bitmap));
...
int32
ctc_goldengate_vlan_set_tagged_ports(uint8 lchip, uint16 vlan_id, ctc_port_bitmap_t port_bitmap)
{
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_VLAN);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_vlan_set_tagged_ports(lchip, vlan_id, port_bitmap));
...
ctc_goldengate_vlan_add_vlan_class(uint8 lchip, ctc_vlan_class_t* p_vlan_class)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,ret,
                                sys_goldengate_vlan_add_vlan_class(lchip, p_vlan_class));
    }

    /*roll back if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_vlan_remove_vlan_class(lchip,p_vlan_class);
    }
    return ret;
}
...
ctc_goldengate_vlan_remove_vlan_class(uint8 lchip, ctc_vlan_class_t* p_vlan_class)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_NOT_EXIST,ret,
                                        sys_goldengate_vlan_remove_vlan_class(lchip, p_vlan_class));
    }
...
ctc_goldengate_vlan_remove_all_vlan_class(uint8 lchip, ctc_vlan_class_type_t type)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_NOT_EXIST,ret,
                                        sys_goldengate_vlan_remove_all_vlan_class(lchip, type));
    }
...
ctc_goldengate_vlan_add_default_vlan_class(uint8 lchip, ctc_vlan_class_type_t type, ctc_vlan_miss_t* p_action)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,ret,
                            sys_goldengate_vlan_add_default_vlan_class(lchip, type, p_action));
    }

    /*roll back if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_vlan_remove_default_vlan_class(lchip,type);
    }
...
ctc_goldengate_vlan_create_vlan_range_group(uint8 lchip, ctc_vlan_range_info_t* vrange_info, bool is_svlan)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_VLAN_RANGE_EXIST,ret,
                                sys_goldengate_vlan_create_vlan_range(lchip, vrange_info, is_svlan));
    }

    /*roll back if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_vlan_destroy_vlan_range(lchip, vrange_info);
    }
...
ctc_goldengate_vlan_destroy_vlan_range_group(uint8 lchip, ctc_vlan_range_info_t* vrange_info)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_VLAN_RANGE_NOT_EXIST,ret,
                                       sys_goldengate_vlan_destroy_vlan_range(lchip, vrange_info));
    }
...
ctc_goldengate_vlan_add_vlan_range(uint8 lchip, ctc_vlan_range_info_t* vrange_info, ctc_vlan_range_t* vlan_range)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_NONE,ret,
                        sys_goldengate_vlan_add_vlan_range_member(lchip, vrange_info, vlan_range));
    }

    /*roll back if error exist*/
    CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
    {
        sys_goldengate_vlan_remove_vlan_range_member(lchip, vrange_info, vlan_range);
    }
...
ctc_goldengate_vlan_add_vlan_mapping(uint8 lchip, uint32 gport, ctc_vlan_mapping_t* p_vlan_mapping)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,ret,
                    sys_goldengate_vlan_add_vlan_mapping(lchip, gport, p_vlan_mapping));
    }
    ...
    if(1 == all_lchip)
    {
        CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
        {
            sys_goldengate_vlan_remove_vlan_mapping(lchip,gport,p_vlan_mapping);
        }
    }
...
*/
int32
ctc_goldengate_vlan_get_vlan_mapping(uint8 lchip, uint32 gport, ctc_vlan_mapping_t* p_vlan_mapping)
{
    uint8 lchip_start              = 0;
    uint8 lchip_end                = 0;
    uint8 all_lchip                = 0;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_vlan_get_vlan_mapping(lchip, gport, p_vlan_mapping));
    }
...
ctc_goldengate_vlan_add_default_vlan_mapping(uint8 lchip, uint32 gport, ctc_vlan_miss_t* p_action)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,ret,
            sys_goldengate_vlan_add_default_vlan_mapping(lchip, gport, p_action));
    }
    ...
    if(1 == all_lchip)
    {
        CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
        {
            sys_goldengate_vlan_remove_default_vlan_mapping(lchip, gport);
        }
    }
    return ret;
}
...
ctc_goldengate_vlan_remove_all_vlan_mapping_by_port(uint8 lchip, uint32 gport)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_NOT_EXIST,ret,
                        sys_goldengate_vlan_remove_all_vlan_mapping_by_port(lchip, gport));
    }
    return ret;
}
...
ctc_goldengate_vlan_add_egress_vlan_mapping(uint8 lchip, uint32 gport, ctc_egress_vlan_mapping_t* p_vlan_mapping)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,ret,
                        sys_goldengate_vlan_add_egress_vlan_mapping(lchip, gport, p_vlan_mapping));
    }
    ...
    if(1 == all_lchip)
    {
        CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
        {
            sys_goldengate_vlan_remove_egress_vlan_mapping(lchip, gport, p_vlan_mapping);
        }
    }
    return ret;
}
...
*/
int32
ctc_goldengate_vlan_get_egress_vlan_mapping(uint8 lchip, uint32 gport, ctc_egress_vlan_mapping_t* p_vlan_mapping)
{
    uint8 lchip_start              = 0;
    uint8 lchip_end                = 0;
    uint8 all_lchip                = 0;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_lchip)
    {
        CTC_ERROR_RETURN(sys_goldengate_vlan_get_egress_vlan_mapping(lchip, gport, p_vlan_mapping));
    }
...
ctc_goldengate_vlan_add_default_egress_vlan_mapping(uint8 lchip, uint32 gport, ctc_vlan_miss_t* p_action)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,ret,
                        sys_goldengate_vlan_add_default_egress_vlan_mapping(lchip, gport, p_action));
    }
    ...
    if(1 == all_lchip)
    {
        CTC_FOREACH_ROLLBACK(lchip_start,lchip_end)
        {
            sys_goldengate_vlan_remove_default_egress_vlan_mapping(lchip, gport);
        }
    }
    return ret;
}
...
ctc_goldengate_vlan_remove_all_egress_vlan_mapping_by_port(uint8 lchip, uint32 gport)
{
    ...
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_NOT_EXIST,ret,
                        sys_goldengate_vlan_remove_all_egress_vlan_mapping_by_port(lchip, gport));
    }
    return ret;
}

int32
ctc_goldengate_vlan_set_mac_auth(uint8 lchip, uint16 vlan_id, bool enable)
{
    uint8 lchip_start              = 0;
    uint8 lchip_end                = 0;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_vlan_set_mac_auth(lchip, vlan_id, enable));
    }
...
int32
ctc_goldengate_vlan_get_mac_auth(uint8 lchip, uint16 vlan_id, bool* enable)
{
    uint8 lchip_start              = 0;
    uint8 lchip_end                = 0;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_vlan_get_mac_auth(lchip, vlan_id, enable));
    }
