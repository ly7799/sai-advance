...
ctc_goldengate_ftm_mem_alloc(uint8 lchip, ctc_ftm_profile_info_t* ctc_profile_info)
{
    ...
    {
        CTC_ERROR_RETURN(sys_goldengate_ftm_mem_alloc(lchip, ctc_profile_info));
    }
...
ctc_goldengate_ftm_mem_free(uint8 lchip)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_ftm_mem_free(lchip));
    }
...
{
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_ftm_get_hash_poly(lchip, hash_poly));
...
ctc_goldengate_ftm_set_hash_poly(uint8 lchip, ctc_ftm_hash_poly_t* hash_poly)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_ftm_set_hash_poly(lchip, hash_poly));
    }
...
ctc_goldengate_ftm_set_profile_specs(uint8 lchip, uint32 spec_type, uint32 value)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_ftm_set_profile_specs(lchip, spec_type, value));
    }
...
{
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(sys_goldengate_ftm_get_profile_specs(lchip, spec_type, value));
