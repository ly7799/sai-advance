...
ctc_goldengate_stacking_replace_trunk_ports(uint8 lchip, ctc_stacking_trunk_t* p_trunk, uint32* gports, uint16 mem_ports)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_stacking_replace_trunk_ports(lchip, p_trunk->trunk_id, gports, mem_ports));

    return CTC_E_NONE;
}

int32
ctc_goldengate_stacking_get_member_ports(uint8 lchip, uint8 trunk_id, uint32* p_gports, uint8* cnt)
{
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_STACKING);
    LCHIP_CHECK(lchip);

    CTC_ERROR_RETURN(sys_goldengate_stacking_get_member_ports(lchip, trunk_id, p_gports, cnt));
...
ctc_goldengate_stacking_get_trunk_rchip(uint8 lchip, ctc_stacking_trunk_t* p_trunk)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_stacking_get_trunk_rchip(lchip, p_trunk));
...
ctc_goldengate_stacking_create_keeplive_group(uint8 lchip, uint16 group_id)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_EXIST,
                                 ret,
                                 sys_goldengate_stacking_create_keeplive_group(lchip, group_id));
...
ctc_goldengate_stacking_destroy_keeplive_group(uint8 lchip, uint16 group_id)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_ENTRY_NOT_EXIST,
                                 ret,
                                 sys_goldengate_stacking_destroy_keeplive_group(lchip, group_id));
    }
    return ret;
}
...
ctc_goldengate_stacking_keeplive_add_member(uint8 lchip, ctc_stacking_keeplive_t* p_keeplive)
{
    ...
    if (p_keeplive->trunk_bmp_en)
    {
        ...
        for (loop1 = 0; loop1 < CTC_STK_TRUNK_BMP_NUM * 32; loop1++)
        {
            ...
            ret = sys_goldengate_stacking_keeplive_add_member(lchip, p_keeplive);
...
Error:
    ...
    ctc_goldengate_stacking_keeplive_remove_member(lchip, p_keeplive);
    return ret;
}
...
ctc_goldengate_stacking_keeplive_remove_member(uint8 lchip, ctc_stacking_keeplive_t* p_keeplive)
{
    ...
    if (p_keeplive->trunk_bmp_en)
    {
        ...
        for (loop1 = 0; loop1 < CTC_STK_TRUNK_BMP_NUM * 32; loop1++)
        {
            ...
            p_keeplive->trunk_id = loop1;
            ret = sys_goldengate_stacking_keeplive_remove_member(lchip, p_keeplive);
...
ctc_goldengate_stacking_keeplive_get_members(uint8 lchip, ctc_stacking_keeplive_t* p_keeplive)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_stacking_keeplive_get_members(lchip, p_keeplive));
    return CTC_E_NONE;
}
...
ctc_goldengate_stacking_set_trunk_mcast_profile(uint8 lchip, ctc_stacking_trunk_mcast_profile_t* p_mcast_profile)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_stacking_set_trunk_mcast_profile(lchip, p_mcast_profile));
...
ctc_goldengate_stacking_get_trunk_mcast_profile(uint8 lchip, ctc_stacking_trunk_mcast_profile_t* p_mcast_profile)
{
    ...
    CTC_ERROR_RETURN(sys_goldengate_stacking_get_trunk_mcast_profile(lchip, p_mcast_profile));
...
ctc_goldengate_stacking_deinit(uint8 lchip)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, 1)
    {
        CTC_ERROR_RETURN(sys_goldengate_stacking_deinit(lchip));
    }
