/**
 ...
 Goldengate supports wire speed IPv4/IPv6 unicast routing under any situation. According to the description
 in l3if module, the l3 source interface should be configed. For both IPv4/IPv6 unicast routing, the source
...
\b
 LPM algorithm
\pp
 Goldengate support two stage LPM lookup. For IPv4, the route with prefix [8-24] will use LPM lookup.
...
\b
\pp
 Goldengate supports uRPF without any limitation. Based on general Unicast routing, there are some additional
...
\S ctc_ipuc.h:ctc_ipuc_flag_t
\S ctc_ipuc.h:ctc_ip_global_property_flag_t
\S ctc_ipuc.h:ctc_ipuc_tunnel_flag_t
\S ctc_ipuc.h:ctc_ipuc_tunnel_payload_type_t
\S ctc_ipuc.h:ctc_ipuc_nat_flag_t
...
#ifndef _CTC_GOLDENGATE_IPUC_H
#define _CTC_GOLDENGATE_IPUC_H
#ifdef __cplusplus
extern "C" {
#endif
...
/**
 ...
 @remark This function add both IPv4 and IPv6 IP unicast entrys. Different properties can be retrived by set different flags
         ...
         ctc_goldengate_ipuc_add_default_entry(uint8 ip_ver, uint32 nh_id).
\p
 @return CTC_E_XXX
...
/**
 ...
 @remark This function set a new default behavior for the vrfid 0 and the ip version which defined by the parameter ip_ver.
         ...
         This function equals to the function ctc_goldengate_ipuc_add(ctc_ipuc_param_t* p_ipuc_param) with mask length 0
         and vrfid 0.
 \p
 @return CTC_E_XXX
...
/**
 ...
 @remark  IP Tunneling is a mechanism for encapsulating one protocol in another and for transport of a packet across a
          network that does not support the native protocol of the packet. Goldengate supports diverse IP tunneling
...
\p
          ...
          ctc_goldengate_nh_add_ip_tunnel(uint32 nhid, ctc_ip_tunnel_nh_param_t* p_nh_param).
\p
...
\p
          Goldengate supports enable tunnel decapsulation on port. The port on which tunnel is terminated should
          enable SCL lookup, more information please refer to function
          ctc_goldengate_port_set_scl_key_type(uint32 gport, ctc_direction_t dir, uint8 scl_id, ctc_port_scl_key_type_t type).
\p
...
*/
extern int32
ctc_goldengate_ipuc_traverse(uint8 lchip, uint8 ip_ver, void* fn, void* data);
...
/**
 @brief Set ipuc nat-da entry aging hit status

 @param[in] lchip    local chip id

 @param[in] p_ipuc_param Input data of the ipuc key

 @param[in] hit Hit status

 @remark Set the IPUC entry hit value.
...
/**
 @brief Get ipuc nat-da entry aging hit status
 ...
 @remark Get the IPUC entry hit value.

 @return CTC_E_XXX

*/
extern int32
ctc_goldengate_ipuc_get_entry_hit(uint8 lchip, ctc_ipuc_param_t* p_ipuc_param, uint8* hit);

/**
 @brief Get ipuc nat-sa entry aging hit status

 @param[in] lchip    local chip id

 @param[in] p_ipuc_nat_sa_param Input data of the ipuc nat key

 @param[out] hit Hit status

 @remark Get the IPUC nat-sa entry hit value.
...
/**
 @brief Set ipuc nat-sa entry aging hit status
 ...
 @remark Set the IPUC nat-sa entry hit value.
