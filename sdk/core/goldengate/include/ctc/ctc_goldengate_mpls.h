...
\p
\b
 In Goldengate, MPLS is treated as a L3 feature, and L3 interface
...
Basic Mpls
\pp
  Goldengate MPLS support the basic packet operation:
\ppp
...
\pp
  Goldengate MPLS support L-Lsp and E-Lsp and three Differentiated Services
  Modes:
\ppp
    \ddd UNIFORM
    \ddd PIPE
...
L2VPN
\pp
 Goldengate MPLS support the point-to-point Virtual Private Wire Service (VPWS)
...
\pp
 Goldengate MPLS support two AC access methods:
    \ddd Ethernet Access
    \ddd VLAN Access
\pp
...
\pp
 ...
 \S ctc_mpls.h:ctc_mpls_property_t
*/

#ifndef _CTC_GOLDENGATE_MPLS_H
#define _CTC_GOLDENGATE_MPLS_H
#ifdef __cplusplus
extern "C" {
#endif
...
/**
 ...
 @remark Initialize the Goldengate MPLS subsystem, Including global variable, interface space size, register callback
                function, etc.
...
\p
                In Goldengate, every ilm is used with an nexthop,the default action for the label is pop.
\b
...
\b
                ...
                in one pkt. But user also can configure a loopback nh,by this way, Goldengate can manage the packet
...
/**
 ...
 @remark Set MPLS properties relating to OAM, QOS or APS
