...
#define SYS_DATAPATH_SERDES_IS_HSS28G(serdes_id) \
    ((((serdes_id) >= 40) && ((serdes_id) <= 47)) || (((serdes_id) >= 88) && ((serdes_id) <= 95)))
...
enum sys_datapath_hssclk_e
{
    ...
    SYS_DATAPATH_HSSCLK_644DOT53125,   /* 644.53125M: for cg, 12.5g, 50g */
...
extern int32 sys_goldengate_datapath_deinit(uint8 lchip);
extern int32 sys_goldengate_datapath_set_serdes_polarity(uint8 lchip, void* p_data);
extern int32 sys_goldengate_datapath_set_serdes_mode(uint8 lchip, uint8 serdes_id, uint8 mode, uint16 overclocking_speed);
...
extern int16 sys_goldengate_datapath_set_serdes_aec_aet_en(uint8 lchip, uint16 serdes_id, uint16 cfg_flag, bool enable, uint32 is_normal);
extern int16 sys_goldengate_datapath_set_serdes_aec_aet_done(uint8 lchip, uint16 serdes_id);
extern int32 sys_goldengate_datapath_get_serdes_ffe(uint8 lchip, uint8 serdes_id, uint16* coefficient, uint8 ffe_mode, uint8* status);
...
extern int16 sys_goldengate_datapath_get_serdes_auto_neg_local_ability(uint8 lchip, uint16 serdes_id, uint32* p_ability);
extern int16 sys_goldengate_datapath_get_serdes_auto_neg_remote_ability(uint8 lchip, uint16 serdes_id, uint32* p_ability);
...
extern int32 sys_goldengate_datapath_set_serdes_tx_en(uint8 lchip, uint16 serdes_id, bool enable);
extern int16 sys_goldengate_datapath_set_serdes_aec_rx_en(uint8 lchip, uint16 serdes_id, uint32 enable);
extern int32 sys_goldengate_datapath_wb_sync(uint8 lchip);
extern int32 sys_goldengate_datapath_serdes_tx_en_with_mac(uint8 lchip, uint8 slice_id, uint8 mac_id);
extern int32 sys_goldengate_datapath_set_dfe_en(uint8 lchip, uint8 serdes_id, uint8 enable);
extern int16 sys_goldengate_datapath_set_serdes_auto_neg_bypass_en(uint8 lchip, uint16 serdes_id, uint32 enable);
extern uint32 sys_goldengate_datapath_get_serdes_no_random_data_flag(uint8 lchip, uint8 serdes_id);
extern int32 sys_goldengate_datapath_get_serdes_mode(uint8 lchip, void* p_data);
extern int32 sys_goldengate_datapath_set_serdes_cfg(uint8 lchip, uint32 prop_type, void* p_data);
extern int32 sys_goldengate_datapath_get_serdes_cfg(uint8 lchip, uint32 prop_type, void* p_data);
extern uint32 sys_goldengate_datapath_get_serdes_aec_lock(uint8 lchip, uint8 serdes_id);
extern uint32 sys_goldengate_datapath_get_serdes_fec_work_round_flag(uint8 lchip, uint8 serdes_id);
extern uint32 sys_goldengate_datapath_get_prbs_status(uint8 lchip, uint8 serdes_id, uint8* enable, uint8* hss_id,uint8* hss_type,uint8* lane_id);
#ifdef __cplusplus
}
#endif
