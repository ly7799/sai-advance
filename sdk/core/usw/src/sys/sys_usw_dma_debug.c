...
sys_usw_dma_show_tx_list(uint8 lchip)
{
    int32 ret = 0;
    uint8 i = 0;
    sys_dma_tx_async_t* p_tx_async = NULL;
    ...
    for(i  = 0; i < 2 ; i++)
    {
        if(i == 0)
        {
            ...
            SYS_DMA_DBG_OUT(CTC_DEBUG_LEVEL_DUMP, "%-20s%-20s\n","packet addr","is zero copy");
            p_tx_async = (sys_dma_tx_async_t*)(p_usw_dma_master[lchip]->tx_pending_list_H->head);
        }
        else
        {
            SYS_DMA_DBG_OUT(CTC_DEBUG_LEVEL_DUMP, "----------------Low priority-------------\n");
            SYS_DMA_DBG_OUT(CTC_DEBUG_LEVEL_DUMP, "%-20s%-20s\n","packet addr","is zero copy");
            p_tx_async = (sys_dma_tx_async_t*)(p_usw_dma_master[lchip]->tx_pending_list_L->head);
        }
        while(p_tx_async)
        {
            SYS_DMA_DBG_OUT(CTC_DEBUG_LEVEL_DUMP, "%-20p%-20d\n",p_tx_async->tx_pkt, p_tx_async->tx_pkt->tx_info.flags & CTC_PKT_FLAG_ZERO_COPY);
            p_tx_async = (sys_dma_tx_async_t*)p_tx_async->node_head.next;
        }
    }
...
sys_usw_dma_set_timer_init_en(uint8 lchip)
{
    ...
    chan_id[0] = SYS_DMA_PKT_TX_TIMER_CHAN_ID;
    chan_id[1] = SYS_DMA_TBL_WR_CHAN_ID;
...
sys_usw_dma_get_dma_info(uint8 lchip, uint32* p_size, void** p_virtual_addr)
{
    ...
    *p_size = dma_info.size;
    *p_virtual_addr = dma_info.virt_base;
