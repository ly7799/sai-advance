        ...
        || (SYS_NH_PARAM_MCAST_MEM_LOCAL_WITH_NH == p_member->member_type))
    {
        ...
        if (!p_member->is_linkagg)
        {
            ...
            else
            {
                ...
                CTC_ERROR_RETURN(DRV_IOCTL(lchip, p_member->destid&0x7FF, cmd, &arp_destmap));
                if (arp_destmap != 0xffff)
                {
                    p_gports[0] = SYS_USW_DESTMAP_TO_DRV_GPORT(arp_destmap);
                    p_gports[0] = SYS_MAP_DRV_GPORT_TO_CTC_GPORT(p_gports[0]);
                    cnt = 1;
                }
            }
        }
...
_sys_usw_nh_mcast_write_dsmet(uint8 lchip, sys_nh_info_dsmet_t* p_met_info)
{
    ...
    */
    ...
    if (CTC_FLAG_ISSET(p_met_info->flag, SYS_NH_DSMET_FLAG_IS_MCAST_APS))
    {
        ...
        CTC_ERROR_RETURN(sys_usw_aps_set_met_entry(lchip, &dsmet_para, p_met_info->basic_dsmet_offset));
        return CTC_E_NONE;
    }
                                        ...
                                        sys_nh_info_mcast_t* p_info_mcast)
{
    ...
    p_member_info->dsmet.entry_type = p_mem_param->fid?SYS_NH_ENTRY_TYPE_MET_6W:SYS_NH_ENTRY_TYPE_MET;
    ...
    if (ctc_list_pointer_empty(p_db_member_list))
    {
        ...
        p_member_info->dsmet.basic_dsmet_offset = basic_met_offset;

        if ((SYS_NH_PARAM_MCAST_MEM_LOCAL_WITH_NH_DESTID == p_mem_param->member_type || p_mem_param->fid)&&!g_usw_nh_master[lchip]->met_mode)
        {
            sys_nh_info_dsmet_t dsmet;
            sal_memset(&dsmet, 0, sizeof(sys_nh_info_dsmet_t));
        ...
        {
            ret = (sys_usw_nh_offset_alloc(lchip, SYS_NH_ENTRY_TYPE_MET, 1, &new_met_offset));
            if (ret)
            {
                SYS_NH_DBG_OUT(CTC_DEBUG_LEVEL_ERROR, "Alloc met offset failed!\n");
                goto error1;
        ...
        if(dsmet_io.met_offset == p_info_mcast->basic_met_offset)
        {
    ...
    else
    {
       if(p_member_info->dsmet.entry_type == SYS_NH_ENTRY_TYPE_MET
        ...
        else
        {
            ...
            ret = (sys_usw_nh_offset_alloc(lchip, p_member_info->dsmet.entry_type, (p_mem_param->is_mcast_aps?2:1), &new_met_offset));
            if (ret)
            {
                SYS_NH_DBG_OUT(CTC_DEBUG_LEVEL_ERROR, "Alloc met offset failed!\n");
                goto error1;
                                    ...
                                    sys_nh_info_mcast_t* p_info_mcast)
{
    ...
    /*Target member is first member*/
    {
        ...
        if (p_mem_target->list_head.p_next || CTC_FLAG_ISSET(p_mem_target->dsmet.flag, SYS_NH_DSMET_FLAG_IS_MCAST_APS))
        {
            ...
            if (p_mem_flex && (p_mem_flex->dsmet.entry_type == SYS_NH_ENTRY_TYPE_MET_6W) && !g_usw_nh_master[lchip]->met_mode)
            {
            ...
            else
            {
                if (CTC_FLAG_ISSET(p_mem_target->dsmet.flag, SYS_NH_DSMET_FLAG_IS_MCAST_APS) && !p_mem_flex)
                {
                    ...
                    CTC_ERROR_RETURN(_sys_usw_nh_mcast_entry_set_default(lchip, p_info_mcast, p_mem_target->dsmet.basic_dsmet_offset, TRUE, TRUE));
                }
                else
                {
                    ...
                    p_mem_flex->dsmet.dsmet_offset = CTC_FLAG_ISSET(p_mem_target->dsmet.flag, SYS_NH_DSMET_FLAG_IS_MCAST_APS)?
                        p_mem_target->dsmet.basic_dsmet_offset:p_mem_target->dsmet.dsmet_offset;
                    p_mem_flex->dsmet.basic_dsmet_offset = p_mem_target->dsmet.basic_dsmet_offset;
                                   ...
                                   sys_nh_info_mcast_t* p_info_mcast)
{
    ...
    if ((device_info.version_id != 3) && DRV_IS_TSINGMA(lchip) && p_mem_param->fid)
    {
    ...
    if (p_mem_param->member_type == SYS_NH_PARAM_MCAST_MEM_LOCAL_WITH_NH)
    {
        ...
        {
            p_mem_param->destid = ((nh_info.dest_map & 0xffff) | 0x800);
        }
    }
...
_sys_usw_nh_mcast_add_dsfwd(uint8 lchip, sys_nh_info_com_t* p_com_db)
{
	...
	if (!CTC_FLAG_ISSET(p_nh_info->hdr.nh_entry_flags, SYS_NH_INFO_FLAG_HAVE_DSFWD))
    {
       ...
       CTC_ERROR_RETURN(sys_usw_nh_offset_alloc(lchip, SYS_NH_ENTRY_TYPE_FWD, 1, &p_nh_info->hdr.dsfwd_info.dsfwd_offset));
    }
                                 ...
                                 sys_nh_info_com_t* p_com_db)
{
    ...
    {
        SYS_NH_UNLOCK;
        ret = REGISTER_API(lchip)->stacking_get_mcast_profile_met_offset(lchip,0, &p_mcast_db->profile_met_offset);
        SYS_NH_LOCK;
                                         ...
                                         sys_nh_param_mcast_member_t* p_member, uint8 is_add)
{
    ...
    if (stk_member_list)
    {
        ...
        else
        {
            ...
            {
                SYS_NH_UNLOCK;
                ret = REGISTER_API(lchip)->stacking_get_mcast_profile_met_offset(lchip, p_member->destid, &first_met_offset);
                SYS_NH_LOCK;
                CTC_ERROR_RETURN(ret);
            }
