...
#define SYS_PKT_U3_OTHER_I2E_SRC_CID_VALID 6
#define SYS_PKT_U3_OTHER_FROM_CPU 7
#define SYS_PKT_U1_OAM_RX_OAM 0
#define SYS_PKT_U2_OAM_MIP_EN 0
#define SYS_PKT_u3_OTHER_DST_CHAN_ID_FROM_CPU 8
#define SYS_PKT_U3_OTHER_DST_CHAN_ID_VALID_RROM_CPU 14
/****************************************************************************
 *
* Functions
*
...
sys_tsingma_packet_txinfo_to_rawhdr(uint8 lchip, ctc_pkt_info_t* p_tx_info, uint32* p_raw_hdr_net, uint8 mode, uint8* data)
{
    ...
    ms_packet_header_tm_t* p_raw_hdr              = (ms_packet_header_tm_t*)bheader;
    uint32 dest_map                = 0;
    uint16 vlan_ptr                = 0;
    uint8 gchip                    = 0;
    uint32 lport                   = 0;
    ...
    p_raw_hdr->u3_data_2 |= (1<<SYS_PKT_U3_OTHER_FROM_CPU); /*u3.other.from_cpu{4,7,1}*/
    p_raw_hdr->logic_src_port_14_6 = (p_tx_info->logic_src_port>>6)&0x1FF;
    p_raw_hdr->logic_src_port_5_0 = p_tx_info->logic_src_port&0x3F;
   ...
   /*Set Vlan domain */
    ...
    if (CTC_FLAG_ISSET(p_tx_info->flags, CTC_PKT_FLAG_ASSIGN_DEST_PORT))
    {
        ...
        if(0xff != chan_id)
        {
             p_raw_hdr->u3_data_2 |= (chan_id<<SYS_PKT_u3_OTHER_DST_CHAN_ID_FROM_CPU);   /* u3_other_destChannelIdFromCpu, 6, {4,8,6}*/
             p_raw_hdr->u3_data_2 |= 1<<SYS_PKT_U3_OTHER_DST_CHAN_ID_VALID_RROM_CPU; /* u3_other_destChannelIdValidFromCpu, 1, {4,14,1}*/
        }
    }
    ...
    {
        p_raw_hdr->u1.normal.source_port_isolateId_0_0 = p_tx_info->isolated_id&0x1;
        p_raw_hdr->u1.normal.source_port_isolateId_6_1 = (p_tx_info->isolated_id>>1)&0x3f;
    }
    /* Linkagg Hash*/
    ...
    if (CTC_PKT_OPER_OAM == p_tx_info->oper_type)
    {
        ...
        {
             p_raw_hdr->u2_data |= (1<<SYS_PKT_U2_OAM_MIP_EN);   /*u2_oam_mipEn, u2_oam_mipEn, 1, {2,8,1}*/
    ...
    if (CTC_PKT_OPER_PTP == p_tx_info->oper_type)
    {
        ...
        p_raw_hdr->timestamp_36_5_16_0 = ts_36_5&0x1FFFF;
        p_raw_hdr->timestamp_36_5_31_17 = (ts_36_5>>17)&0x7FFF;
...
#endif
#ifndef CTC_PKT_DIS_STK
    if (CTC_PKT_OPER_C2C == p_tx_info->oper_type)
    {
        uint8  sub_queue_id = 0;
        uint8  grp_id       = 0;
        p_raw_hdr->logic_src_port_14_6 = 0; /*logic port mean c2c extend group id*/
        p_raw_hdr->logic_src_port_5_0 = 0;
        p_raw_hdr->bypass_ingress_edit = 1;
        if (CTC_FLAG_ISSET(p_tx_info->flags, CTC_PKT_FLAG_MCAST))
        {
...
sys_tsingma_packet_rawhdr_to_rxinfo(uint8 lchip, uint32* p_raw_hdr_net, ctc_pkt_rx_t* p_pkt_rx)
{
    ...
    sys_usw_dword_reverse_copy((uint32*)bheader, (uint32*)p_raw_hdr_net, SYS_USW_PKT_HEADER_LEN/4);
    sys_usw_swap32((uint32*)p_raw_hdr, SYS_USW_PKT_HEADER_LEN / 4, FALSE);
    ...
    p_rx_info->logic_src_port  = (p_raw_hdr->logic_src_port_14_6<<6)|p_raw_hdr->logic_src_port_5_0;
    ...
    if (CTC_PKT_OPER_NORMAL == p_rx_info->oper_type)
    {
        if (p_raw_hdr->u1.normal.cvlan_tag_operation_valid)
        {
            ...
            /*u3_other_i2eSrcCidValid, 1, {4,6,1} */
            /*u3_other_i2eSrcCid, u3_other_i2eSrcCid, 8, {4,0,6}, {3,30,2}*/
     ...
     {
         p_rx_info->cid = (((p_raw_hdr->u3_data_2&0x3F)<<2) | ((p_raw_hdr->u3_data_1>>7)&0x3));
     }

#ifndef  CTC_PKT_DIS_TS
    /* Timestamp */
    ts_36_5 = (p_raw_hdr->timestamp_36_5_31_17<<17) | p_raw_hdr->timestamp_36_5_16_0 ;
    ...
    if (CTC_PKT_OPER_OAM == p_rx_info->oper_type)
    {
        ...
        {
            p_rx_info->oam.lm_fcl = (p_raw_hdr->u3_data_2<<9) | p_raw_hdr->u3_data_1; /*u3_oam_rxtxFcl, 24, {4,0,15}, {3,23,9}*/
...
#ifndef CTC_PKT_DIS_WLAN
    if(2 == p_rx_info->oper_type)  /* OPERATIONTYPE_E2ILOOP */
    {
