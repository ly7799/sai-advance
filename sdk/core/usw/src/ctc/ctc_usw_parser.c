...
ctc_usw_parser_set_global_cfg(uint8 lchip, ctc_parser_global_cfg_t* global_cfg)
{
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_chip)
    {
        CTC_ERROR_RETURN(MCHIP_API(lchip)->parser_set_global_cfg(lchip, (ctc_parser_global_cfg_t*)global_cfg));
    }
...
ctc_usw_parser_get_global_cfg(uint8 lchip, ctc_parser_global_cfg_t* global_cfg)
{
    ...
    FEATURE_SUPPORT_CHECK(CTC_FEATURE_REGISTER);
    LCHIP_CHECK(lchip);
    CTC_ERROR_RETURN(MCHIP_API(lchip)->parser_get_global_cfg(lchip, (ctc_parser_global_cfg_t*)global_cfg));
    return CTC_E_NONE;
}
...
ctc_usw_parser_init(uint8 lchip, void* parser_global_cfg)
{
    ...
    {
        ret = sys_usw_parser_init(lchip, parser_global_cfg);
        if (ret && (ret != CTC_E_PARTIAL_PARAM))
        {
