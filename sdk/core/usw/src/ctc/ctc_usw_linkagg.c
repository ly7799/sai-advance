...
/**
 @brief The function is to add  port  array to linkagg group

 @param[in] tid the linkagg id wanted to operate

 @param[in] member_cnt the port number in the port-array

 @param[in] p_port_array  address of the buffer contained the each port information
 
 @return CTC_E_XXX

*/
int32
ctc_usw_linkagg_add_ports(uint8 lchip, uint8 tid, uint32 member_cnt, ctc_linkagg_port_t* p_ports)
{
    uint8 lchip_start = 0;
    uint8 lchip_end = 0;
    int32 ret = CTC_E_NONE;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_chip)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_NOT_EXIST,
            ret,
            sys_usw_linkagg_add_ports(lchip, tid, member_cnt, p_ports));
    }
    return ret;
}

/**
 @brief The function is to remove  port  array to linkagg group
...
*/
int32
ctc_usw_linkagg_remove_ports(uint8 lchip, uint8 tid, uint32 member_cnt, ctc_linkagg_port_t* p_ports)
{
    uint8 lchip_start = 0;
    uint8 lchip_end = 0;
    int32 ret = CTC_E_NONE;
    ...
    CTC_FOREACH_LCHIP(lchip_start, lchip_end, all_chip)
    {
        CTC_FOREACH_ERROR_RETURN(CTC_E_NOT_EXIST,
            ret,
            sys_usw_linkagg_remove_ports(lchip, tid, member_cnt, p_ports));
    }
    return ret;
}
