...
#define CTC_CLI_MPLS_COMM_OPT \
    "stats STATS_ID |trust-outer-exp |qos-use-outer-info | acl-use-outer-info | flex-edit |eslb-exist |"
        ...
        "Is tcam")
{
    ...
    idx = CTC_CLI_GET_ARGC_INDEX("eslb-exist");
    if (0xFF != idx)
    {
        CTC_SET_FLAG(ilm_info.flag, CTC_MPLS_ILM_FLAG_ESLB_EXIST);
    }
