...
*/
int32
drv_goldengate_model_write_sram_entry(uint8 chip_id_offset, uintptr addr, uint32* data, int32 len);
/**
 @brief read table data to a sram memory location on memory model(according to one address)
*/
int32
drv_goldengate_model_read_sram_entry(uint8 chip_id_offset, uintptr addr, uint32* data, int32 len);
