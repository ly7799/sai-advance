...
/*#include "goldengate/include/drv_tbl_reg.h"*/
...
***********************************************************************************/
#define DRV_IOCTL           drv_goldengate_ioctl_api
#define DRV_TCAM_TBL_REMOVE drv_goldengate_tcam_tbl_remove
#define DRV_HASH_KEY_IOCTL  drv_goldengate_hash_key_ioctl
#define DRV_HASH_LOOKUP     drv_goldengate_hash_lookup
#define DRV_SET_FIELD_ADDR  drv_goldengate_set_field
#define DRV_GET_FIELD       drv_goldengate_get_field


#define DRV_SET_FIELD_V(tbl_id, field_id, entry, value) \
    {                                                       \
        uint32 vl = value;                                  \
        drv_goldengate_set_field(tbl_id, field_id, entry, &vl);        \
    }
#define DRV_SET_FIELD_A(t,f,d,v)    drv_goldengate_set_field(t,f,d,(uint32*)v)
#define DRV_GET_FIELD_V             drv_goldengate_get_field_value
#define DRV_GET_FIELD_A(t,f,d,v)    drv_goldengate_get_field(t,f,d,(uint32*)v)
...
typedef struct
{
	...
	uint32 wb_status[MAX_LOCAL_CHIP_NUM];  /*1:done; 2:reloading; 3:sync*/
...
*/
extern int32
drv_goldengate_hash_key_ioctl(void*, hash_op_type_t);
...
*/
extern int32
drv_goldengate_get_tcam_lookup_bitmap(uint32 mem_id, uint32 *bitmap);

/**
 @brief set tcam lookup bitmap
*/
extern int32
drv_goldengate_set_tcam_lookup_bitmap(uint32 mem_id, uint32 bitmap);

extern int32
drv_goldengate_get_hash_lookup_bitmap(uint32 hash_module, uint32 *bitmap, uint32 *extra_mode);
extern int32
drv_goldengate_set_hash_lookup_bitmap(uint32 hash_module, uint32 bitmap, uint32 extra_mode);

extern int32
drv_goldengate_get_dynamic_ram_couple_mode(uint32 *couple_mode);
extern int32
drv_goldengate_set_dynamic_ram_couple_mode(uint32 couple_mode);
